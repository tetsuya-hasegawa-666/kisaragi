...
この文書は、`prj-kisaragi_0001` の BDD terminal behavior に対する TDD 計画を管理する正本計画書である。

## 文書の位置付け

- 実装前に test target、検証方法、受け入れ条件、小 milestone の close 条件を明示する正本とする。
- `MRL` と `mRL` の release-line 管理は、この文書の task 分解と evidence 設計を補助する参考情報として扱う。
- test planning と verification planning の正本はこの文書と `bdd-release-compass.md` とし、release-line 文書だけで代替しない。
- release-line の附番は `2026-03-25` の今回修正開始を `MRL-1` / `mRL-1.1` の起点とする。
- `prj-kisaragi_0001` は `codev-viewer` を基底に再構成した project として test planning を引き継ぐ。
...
| `T5` | `B2` | dual-pane independent state | 2 pane が独立表示され、left pane profile switch が成立する | pass | `--devs/--testcode/prj-kisaragi_0001/tests/shared-core/dashboardController.test.ts` |
| `T6` | `B1` | default basis selection | available profiles が返り、default が `prj-view` になる | pass | `--devs/--testcode/prj-kisaragi_0001/tests/document-workspace/documentWorkspaceController.test.ts` |
...
| `T8` | `B3` | compact tree and profile tabs view | profile tab と compact expandable tree を表示できる | pass | `--devs/--testcode/prj-kisaragi_0001/tests/document-workspace/documentWorkspaceView.test.ts` |
| `T9` | `B3` | sticky trail persistence | `prj-kisaragi_0001` を sticky trail で可視に保てる | pass | `--devs/--testcode/prj-kisaragi_0001/tests/document-workspace/documentWorkspaceView.test.ts` |
...
| `T15` | `B9` | manifest path env override | launcher が指定した manifest path を優先できる | pass | `--devs/--testcode/prj-kisaragi_0001/tests/shared-core/liveProjectSnapshot.test.ts` |

## 実行方針

- 1 task 1 責務で進める
- `MRL-1` は `T1` から `T7` で source profile と dual-pane 基線を固める
- launcher の `db-view` / `prj-view` mode は `T2a` と `T15` で固定する
- `MRL-2` は `T8` から `T11` で比較閲覧 UX を固める
- `MRL-3` は `T3`、`T4`、`T12`、`T13` で handoff と read-only 境界を固定する
- `MRL-4` は `T14` で launch と build の継続性を確認する

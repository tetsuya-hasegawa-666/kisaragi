...
| --- | --- | --- | --- | --- | --- |
| `T1` | `B10` | Python session parser alias compatibility | `bt.jsonl` / `poses.jsonl` と legacy alias の両方を 1 parser で読める | pass | `kisaragi-db/--devs/--testlogs/prj-reviework/reports/python-unittest-summary.md` |
| `T2` | `B1` | SessionPackage intake summary | required / optional input の充足状況を summary に落とせる | planned | `none yet` |
| `T3` | `B2` | Thin Status diagnose formatter | `phase`、`pipeline`、`data_health`、`quality`、`issues` の 5 カテゴリで理由を返せる | in_progress | `kisaragi-db/--devs/--products/prj-reviework/app/src/main/java/com/reviework/app/ReviewScreenController.kt` |
| `T4` | `B3` | execute readiness gate | readiness 未達時は `処理を開始` を返さず、修正 action を返す | planned | `none yet` |
| `T5` | `B4` | run phase single-action UX | run phase の `Next Action` が常に 1 件で、current step を別 line に出せる | pass | `kisaragi-db/--exsams/prj-reviework/reports/testDebugUnitTest/junit/TEST-com.reviework.app.ReviewScreenControllerTest.xml` |
...
| `T8` | `B7` | attention point synthesis | attention point に時間範囲と理由を持たせる | in_progress | `kisaragi-db/--exsams/prj-reviework/reports/testDebugUnitTest/junit/TEST-com.reviework.app.ReviewScreenControllerTest.xml` |
...
| `T12` | `B12` | output routing scripts | Android build cache と raw test report が `--exsams`、summary が `--testlogs` に分離される | pass | `kisaragi-db/--devs/--products/prj-reviework/scripts/run_android_unit_tests.ps1` |

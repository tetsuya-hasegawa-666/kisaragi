...
この文書は `prj-reviework` の `current_state`、BDD、TDD を 1 つに統合した正本とする。
...
この章は `prj-reviework` の提供価値、`Purpose Story`、`System Behaviors`、`MRL` / `mRL` の対応を定義する。
...
この章は `prj-reviework` の BDD `System Behaviors` を、1 task 1 責務で実装と検証へ落とす。

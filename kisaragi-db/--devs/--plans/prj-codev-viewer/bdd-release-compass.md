...
9. operator は `npm run dashboard` と launcher icon を入口に、`codev-viewer` として dashboard を起動できる
...
- `B9`: dashboard は `npm run dashboard` と launcher icon を entry にし、browser page title を `codev-viewer` に固定する
...
| `B9` | launch contract | `npm run dashboard` または launcher icon で dashboard が起動し、page title が `codev-viewer` になる |

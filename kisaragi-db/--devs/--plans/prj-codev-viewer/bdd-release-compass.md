...
この文書は、`prj-codev-viewer` の user value、core story、terminal behavior、release line の対応をまとめる。

## ノーススター

operator は `codev-db` と `codev-view` を dual-pane の read-only viewer で安全に比較し、basis 切替、構造確認、文書読解、Explorer handoff を同一画面で迷わず進められる。
...
1. operator は `codev-db` と `codev-view` を basis 単位で切り替えられる
...
9. operator は `npm run dashboard` を入口に、`codev-viewer` として dashboard を起動できる

## terminal behaviors

- `B1`: viewer は複数 source profile の文書を収集し、default basis を `codev-view` として返す
...
- `B9`: dashboard は `npm run dashboard` を entry command とし、browser page title を `codev-viewer` に固定する

## 受け入れ基準

| behavior_id | 観点 | 受け入れ基準 |
| --- | --- | --- |
| `B1` | source profile | 複数 profile の文書収集結果に profile metadata が入り、default basis が `codev-view` で読める |
| `B2` | dual-pane state | 左右 pane の切替や選択が互いの状態を上書きしない |
| `B3` | navigation UI | profile tab、compact tree、sticky trail が同一 workspace で読める |
...
| `B9` | launch contract | `npm run dashboard` で dashboard が起動し、page title が `codev-viewer` になる |

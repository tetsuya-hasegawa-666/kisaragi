# temp DA3 Giant Colab Evid Runbook

## 文書の役割

- この文書は `prj-kisaragi_0002` の `MRL-9` 用に、`DA3 Giant` / `Giant Large` の `infer_gs=True` route を一時的に積み上げる仮正本 runbook とする。
...
- notebook 系 runbook の shared rule に従い、この file は `temp-da3Giant_colab_ref_runbook.md` と対で管理する。周辺説明、OK 条件、採用判断、補助情報は本 file に残し、貼り付け専用 block は companion 側にも同期する。
- 既存の [da3_colab_evid_runbook.md](/Users/tetsuya/kisaragi/kisaragi-db/--devs/--products/prj-kisaragi_0002/modeling/da3_colab_evid_runbook.md) は `MetricLarge route` の正本として維持し、この file では上書きしない。

## この temp runbook の位置づけ

- rollback baseline は `MetricLarge route` とする。
- `MRL-9` の first config は `fps = 1`、`frames = 60`、`process_res = 504`、`chunk = 20` または `30` とする。
- first target は `infer_gs=True` を受ける entrypoint、model id、`gs_ply` / `gs_video` の保存 contract を固定することである。
- top camera renderer、path overlay、reviewing viewer 統合はこの file の範囲外とし、後続 `MRL-**` へ送る。

## 現在の採用方針

- startup、Drive input 選択、install、`MetricLarge route` で確定済みの blank runtime bootstrap は、当面この temp runbook でも同じ構成を流用する。
- `Giant` 固有差分は `MRL-9` section として末尾へ追記し、共有 log で通った内容だけを同期する。
- `MetricLarge route` 正本の本文説明を汚さないため、この file だけに `Giant` 固有の補助説明、blocker、暫定 command を保持してよい。
...
- この section は `MetricLarge route` と共通の startup、Drive input select、install、selected input handoff が通った後に使う。
- blank runtime から始める時も、先頭から `準備確認 1`、`準備確認 2`、`準備確認 3`、`準備確認 4`、`準備確認 5`、`install 1`、`install 2`、`install 3`、`install 4` を実行してから、この section へ入る。
- つまり、揮発 runtime 前提でも temp runbook 単体で blank bootstrap から `MRL-9` 固有 step へ到達できる。

### Step 9a: giant `infer_gs=True` entrypoint probe
...
### Adopted block: giant `infer_gs=True` first export
...
- 重要:
  ...
  - 既に `depth_anything_3` を import 済みの runtime では stale import で `matrix_to_angles` 未定義が残るので、fresh runtime か module reload が必要
- 保存先:
  - `/content/drive/MyDrive/trajectreview/modeling/<session_id>_da3giant_infergs_probe_v01`
- 期待生成物:
  - `gs_ply/0000.ply`

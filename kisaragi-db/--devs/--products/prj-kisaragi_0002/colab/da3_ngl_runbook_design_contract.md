...
- `#6 Shared Helpers` を camera trajectory / pose / intrinsics / anchor basis の唯一の共通契約面とし、`#9`、`#10`、`#11`、`#7`、`#8`、`#13`、`#14` が同じ参照面で動くようにする。
- 実行順の基本は `#6 -> #9 -> #10 -> #11 -> #7 -> #8 -> #13 -> #14` とし、`#7 Full Prepose Build` は anchor stage として chunk-local `DA3 NGL` pose build、overlap matching、chunk graph build、可視化確認までを内包する。
...
- `#7-6` は 2 chunk overlap pose を同一座標系へ再現する matching 面とし、chunk A/B の `pred_extrinsics.npy` と `chunk_input_frames.csv` から overlap 対応、relative transform、matching residual を固定 artifact として残す。
- `#7-7` は route compare の結果を `prepose_chunk_graph_solution_arc.csv`、`prepose_chunk_graph_edges_arc.csv`、`prepose_chunk_graph_summary.json`、`premerge_pose_validation.json` として固定し、`#14-1` はその graph artifact を優先消費する。graph artifact には `graph_parent_chunk_name`、`relative_scale`、`relative_translation_norm`、`relative_rotation_deg` を含める。
- `#7-6` は matching 結果を `overlap_pair_metrics_arc.csv`、`trajectory_points_arc.csv`、`transform_b_to_a.npy`、`trajectory_match.png`、`trajectory_match.html`、`matching_summary.json` として `persist_root/01_anchor/07matching/` に固定する。summary には `relative_scale`、`relative_translation_norm`、`relative_rotation_deg`、pre/post residual を含める。
...
- `#7-6` は `#7` の内部診断面であり、input path が未指定または chunk artifact が未生成の時は `status: skipped` と `reason: matching_inputs_missing` を返して top-to-bottom 実行を壊さない。
- `#7-7` は pre-merge judge / graph build 面とし、最低でも `route_label`、`fallback_used`、`center_rmse`、`rotation_dir_residual`、`owner_record keep` に関わる judge 指標を baseline / experimental の両 route で比較できるように保つ。比較結果は `premerge_route_compare_arc.csv` と `premerge_route_compare_summary.json` に残す。
- `#7-7` はさらに `chunk_to_world` の事前整合 graph artifact を出し、`chunk_name`、`graph_parent_chunk_name`、`route_label`、`fallback_used`、`t00..t33`、overlap / non-overlap residual 指標、`relative_scale`、`relative_translation_norm`、`relative_rotation_deg` を残す。
- `#13-1` は `#7-7` で作られた `premerge_pose_validation.json`、`premerge_route_compare_summary.json`、`prepose_chunk_graph_solution_arc.csv`、`prepose_chunk_graph_edges_arc.csv` を review して `#14-1` へ渡す thin gate とする。

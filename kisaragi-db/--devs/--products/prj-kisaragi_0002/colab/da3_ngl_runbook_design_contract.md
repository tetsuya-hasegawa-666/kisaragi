...
- `#10-1` と `#11-1` の final access contract は `probe_root/final_outputs/#10-1/`、`probe_root/final_outputs/#11-1/` を stage root とし、その直下を `re_access/` と `persist_only/` に固定する。`persist_only/` が canonical file 実体、`re_access/` が後段再参照用 manifest / pointer / summary とする。
- `#10-1` は graph build / gate の canonical file 実体を `#10-1/persist_only/` に保存し、`#10-1/re_access/graph_contract_manifest.json` で consumer 向け read path を明示する。`#11-1` は `full graph solution` のような旧前提を推測せず、この manifest か同じ contract key を持つ summary を先に読む。
- `#11-1` は merge / review の canonical file 実体を `#11-1/persist_only/merged`、`diagnostics`、`manifests`、`chunk_evidence` に保存し、`#11-1/re_access/` には `11_2_handoff_manifest.json`、`11_3_handoff_manifest.json`、`merge_resume_state.json`、`stage_access_index.json` のような再開導線だけを置く。同系統 file を `merged_dir`、`stage_11_2_dir`、`stage_11_3_dir`、`final_outputs_*` へ多重 copy しない。

...
- `#08-3` は planning の管理 file を `pipeline_root/manifests/chunk_execution_plan.csv` へ統合する。target subset、execution batch、chunk scope path は同 file の列で表し、旧 CSV 群は互換派生に留める。
...
- `#11-1` は merge / review の canonical file 実体を `#11-1/persist_only/merged`、`diagnostics`、`manifests`、`chunk_evidence` に保存し、stage 管理 file は `#11-1/persist_only/merge_output_report.json` に統合する。`#11-1/re_access/` には `11_2_handoff_manifest.json`、`11_3_handoff_manifest.json`、`merge_resume_state.json`、`stage_access_index.json` のような再開導線だけを置く。同系統 file を `merged_dir`、`stage_11_2_dir`、`stage_11_3_dir`、`final_outputs_*` へ多重 copy しない。`re_access/` が持つのは path pointer と stage summary だけであり、関係定義そのものは持たない。

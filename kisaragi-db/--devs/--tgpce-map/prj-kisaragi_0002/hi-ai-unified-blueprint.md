...
| `modeling` | `MRL-3` から `MRL-9` は `p-done` である。`MRL-7` は `mRL-7.1` と `mRL-7.2` が `p-done` になり、multi-frame point cloud から gaussian short optimization まで到達した。`MRL-8` では runbook 本体の前段に Drive input candidate scan / widget select を追加し、selected input を bootstrap 本体と `MRL-7` one-block の両方へ handoff できる状態を閉じた。`MRL-9` は `DA3NESTED-GIANT-LARGE-1.1` を使う living route で `infer_gs=True`、`process_res = 504`、`ref_view_strategy = middle`、`num_max_points = 1250000` を与え、`gs_ply`、`gs_video`、`scene.glb`、`debug_gs_readback` bundle を保存し、さらに `gs_ply/0000.ply` を `PlayCanvas Model Viewer` で開けるところまで進んだ。現在の主対象は `MRL-10` であり、`frame_record.jsonl + images` を正本にした record-native `Colab` route に、`correcting` 側の `90度右回転` canonical image 契約、legacy intrinsics 補正、`orientation_summary.json`、`DA3NESTED-GIANT-LARGE-1.1` の living spec に加えて、OOM を避ける `continuous_gs_v06_chunk18_overlap6_adopt12` の `global camera matrix + batch plan -> 3chunk batch 実行 -> final rebuild merge` route を main runbook pair へ吸収し、さらに center-only / PCA 帯 merge による天地反転と pose ずれ、往復経路の混線を、pose-aware alignment 維持 + owner_record ベース merge で解消する段である |
...
| id | blocker | 影響 | 現在の扱い |
| --- | --- | --- | --- |
| `BLK-1` | `MRL-7` の範囲に `TraceCore` 最小表示、route 比較、viewer 後続論点が混在している | `MRL` closeout と `BDD` の焦点がぼける | `TraceCore` 最小表示だけを `MRL-7` の中心に寄せ、後続は `MRL-**` へ分離する |
| `BLK-2` | sampling / intrinsics の最終採用 route が未決定 | `modeling` の採用 route 固定が止まる | 比較基盤、比較実験、採用 route 運用化の 3 段で閉じる前提を維持する |
...
| `BLK-11` | `Giant` viewer / render の image 向きが左に `90度` 倒れた状態で扱われていた | `viewer` 確認、top camera 後続、経路重畳の読解性が落ち、`1 record` 契約に対する回転補正責務も曖昧だった | `MRL-10` へ吸収して解消中。`correcting` から `90度右回転` 済み upright image を canonical input とし、`Block 1` で legacy intrinsics だけを補正し、`orientation_summary.json` と `k_resize_check.csv` へ記録する |
| `BLK-12` | `continuous_gs_v06_chunk18_overlap6_adopt12` の merge が center-only / PCA 帯 keep 前提のままだと、同経路反復や往復を frame 責務で分離できず、camera orientation も十分に使われない | chunk trajectory がほぼ直線に近い session では回転自由度が余り、`merged_gs.ply` / `merged_scene.glb` が天地反転したり camera pose と scene がずれてぶれたように見え、往復経路では chunk 境界が混線しやすい | `MRL-10` に `mRL-10.4` を追加し、global / local の camera matrix 両方を使う pose-aware alignment は維持したまま、merge keep を `owner_record_index` ベースへ置換する。rollback point は `codex/mrl10-merge-baseline-20260404` とし、baseline merge を戻せる状態を維持する |
...
- final merge 後の cleanup は `#11` で yes/no を問い、Drive 側では `chunk_runs/`、local 側では runbook tmp JSON、展開 input、local zip などの不可視生成物だけを削除対象にする。`probe_root` 配下の final / proof / manifest / merged 証跡は保持する。
...
4. `MRL-10` として、`frame_record.jsonl + images` を正にした record-native `Colab` route を canonical 化し、`proof route` と `production route` を分けたうえで `MetricLarge route`、`Giant route`、download bundle を同じ contract にそろえる
...
| `td14a` | `bd16a` | Drive input candidate scan and select | runbook が Drive 上の session zip / session folder 候補を列挙し、selected input を固定して、後続 `Step 1` と `MRL-7` one-block が hardcoded path なしで同じ入力を読める | pass | `kisaragi-db/--devs/--products/prj-kisaragi_0002/modeling/da3_colab_evid_runbook.md` |
...
| `MRL-10` | `-` | `frame_record.jsonl + images` を正とする record-native `Colab` route を canonical 化し、`MetricLarge route` と `Giant route` の両方を同じ manifest 契約で扱える main runbook pair を成立させる。proof と production を分離し、QC、baseline thinning、`debug_gs_readback`、upright orientation 整合を含む評価 manifest を備える | `sd9`,`su12`,<br>`su14`,`sd11` | `bd16`,`bd17`,<br>`bd18a`,`bd20` | `td16`,`td17`,<br>`td18`,`td19a`,<br>`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` section | `未収載` |
| `MRL-10` | `mRL-10.1` | `frame_record.jsonl`、対応 image 群、tracking / K / pose / timestamp を正に読んで、`input_frame_manifest.csv`、`input_frame_qc.csv`、`pose_conversion_check.csv`、`k_resize_check.csv`、`orientation_summary.json`、`da3_input_manifest_*.csv` を生成できる | `sd9`,`sd11` | `bd16`,`bd17`,<br>`bd20` | `td16`,`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Phase A-C | `未収載` |
| `MRL-10` | `mRL-10.2` | proof / production を分離した `MetricLarge route` が image-only 推論で通り、production 側では `frame_record` 由来の `intrinsics` / `extrinsics_w2c` を使って world point cloud と評価 summary まで保存できる。upright canonical image と legacy intrinsics 補正結果が同じ manifest 群で残る | `sd9`,`su14`,<br>`sd11` | `bd17`,`bd18`,<br>`bd20` | `td17`,`td18`,<br>`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Phase D | `未収載` |
| `MRL-10` | `mRL-10.3` | `Giant` production candidate route は `continuous_gs_v06_chunk18_overlap6_adopt12` とし、`Block 3` で `extrinsics_w2c_prod.npy` から全 frame の global camera matrix、`camera_matrix_full.csv`、全 chunk manifest、`batch_plan.csv` を作る。その後 `chunk_size = 18`、`step = 12`、`adopt_size = 12`、`chunks_per_batch = 3`、`process_res = 504` で `Block 4` helper を読み、各 chunk は `6frame` 重複しつつ基本 `後半 12frame` を再構成責務とする。`Block 5` は `RUN_BATCH_INDEX` を変えながら `3chunk` ずつ繰り返して `infer_gs=True` を回す。最後は `Block 6` が completed chunk 全体から `merged_gs.ply` と `merged_scene.glb` を再構築し、Drive では `probe_root` 1 directory に集約保存し、download 用 zip は local `/content` 側だけに作る。出力後は cleanup plan を表示し、不可視生成物だけを削除できる | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Block 3-6 | `未収載` |
| `MRL-10` | `mRL-10.4` | chunk merge は local / global の camera center だけでなく `camera_matrix_full.csv` と各 chunk の `pred_extrinsics.npy` から得る camera orientation も使って similarity を解く。その上で keep 判定は `PCA 1軸帯` を廃止し、各 Gaussian を `top-k` frame への `distance + direction + blur_penalty + index_penalty` score で `owner_record_index` へ帰属させ、owner が当該 chunk の `is_adopted_region=True` record に属する時だけ keep する。owner 候補 frame はまず当該 chunk の record 群だけを使い、候補不足の時だけ `OWNER_RECORD_MARGIN` 付き局所近傍、最後に全体 fallback へ広げる。これにより overlap chunk の vertex が chunk 外 record へ一方的に吸われて `keep_zero_chunk` になる現象を防ぐ。`keep_zero_chunk` は hard error とし、`chunk_global_transforms.csv`、`vertex_assignment_summary.csv`、`owner_record_histogram.csv`、`chunk_assignment_summary.csv`、`merge_warning_summary.json`、`chunk_transform_quality.csv` を残せる | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Block 4-6 | `未収載` |
...
| `mRL-10.4c` | `PCA 1軸帯 keep` と terminal 全保持 fallback を削除し、`owner_record_index` ベース keep へ置き換える。owner 候補は chunk 内 record を優先し、候補不足の時だけ局所 margin / 全体 fallback へ広げる | `active` |
| `mRL-10.4d` | `vertex_assignment_summary.csv`、`owner_record_histogram.csv`、`chunk_assignment_summary.csv`、`merge_warning_summary.json`、`chunk_transform_quality.csv` を Drive bundle / local bundle に残す | `active` |
| `mRL-10.4e` | admin viewer 再確認で天地反転なし、camera pose との二重像なし、往復経路の混線低下を確認する | `ready` |

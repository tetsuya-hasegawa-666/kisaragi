...
| `BLK-4` | `P4` | `adjacent` | `Colab` runbook の canonical route がまだ旧 proof 設計の残骸を引きずっており、sequence-anchor first、adjacent continuity precheck、official API 実行、pre-merge gate が文書・pair・app 既定値で完全一致していない | `1 record` 化した data を `DA3` へ渡す前に `Colab` 側で整合を壊し、`DA3NESTED-GIANT-LARGE-1.1` canonical route の本番品質を落とす | `mRL-10.1` と `mRL-10.2` は `p-done` として前進したが、runbook の `.md/.ipynb` pair と admin closeout は継続更新する。関連 gate は `MRL-10`,`mRL-10.1`,`mRL-10.2`,`mRL-10.5` | `small-open` | `hi-ai-unified-blueprint.md`, `da3_ngl_runbook.md` |
...
| `MRL-10` | `mRL-10.5` | canonical runbook pair 自体の設計を固定する。対象は [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) / [da3_ngl_runbook.ipynb](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.ipynb) であり、`#1`-`#17` の clean bootstrap 構成、section と code cell 番号一致、official API / CLI 基準、`extrinsics_w2c.npy` provenance 明示、anchor 以降の各出力に対する input / output summary 表示、admin 手順と artifact 契約の同期を維持する | `sd9`,`su12`,<br>`sd11` | `bd17`,`bd20` | `td16`,`td17`,<br>`td23` | `active` | `active` | [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) の<br>`#1`-`#17` と<br>admin 手順 38-45 | `未収載` |
...
| `mRL-10.5c` | canonical runbook pair の構成自体を `#1`-`#17` の clean bootstrap runbook として固定し、section 見出しと code cell 番号を一致させ、`extrinsics_w2c.npy` provenance と stage ごとの input / output summary を notebook 上で常時確認できるようにする | `active` |

...
| `BLK-4` | `P4` | `adjacent` | `Colab` runbook の canonical route がまだ `frame_pose_index.csv` 中心、固定枚数 cap、K / pose 未入力の旧 proof 設計を引きずっている | `1 record` 化した data を `DA3` へ渡す前に `Colab` 側で整合を壊し、`MetricLarge route` と `Giant route` の本番品質を落とす | `mRL-10.1` と `mRL-10.2` は `p-done` として前進したが、runbook の `.md/.ipynb` pair と admin closeout は継続更新する。関連 gate は `MRL-10`,`mRL-10.1`,`mRL-10.2` | `small-open` | `hi-ai-unified-blueprint.md`, `da3_colab_evid_runbook.md` |
...
| `MRL-10` | `-` | `frame_record.jsonl + images` を正とする record-native `Colab` route を canonical 化し、`MetricLarge route` と `Giant route` の両方を同じ manifest 契約で扱える main runbook の `.md/.ipynb` pair を成立させる。`mRL-10.1` と `mRL-10.2` は `p-done`、残りは chunk route と merge 品質である | `sd9`,`su12`,<br>`su14`,`sd11` | `bd16`,`bd17`,<br>`bd18a`,`bd20` | `td16`,`td17`,<br>`td18`,`td19a`,<br>`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` section | `未収載` |

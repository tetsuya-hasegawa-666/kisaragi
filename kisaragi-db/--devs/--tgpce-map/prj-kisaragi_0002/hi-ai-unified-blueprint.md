...
| `modeling` | `MRL-3` から `MRL-9` は `p-done` である。`MRL-10` では、`mRL-10.1` と `mRL-10.2` は `frame_record.jsonl + images` を正にした record-native input 正規化、QC、proof / production 分離、world point cloud 保存まで到達しており `p-done` とみなす。`mRL-10.3` は `continuous_gs_v06_chunk18_overlap6_adopt12` の `global camera matrix -> camera_matrix_full_arc.csv -> chunk manifest -> batch_plan -> chunk batch 実行` と、target 2 chunk の `merged_gs_arc.ply` / `merged_scene_arc.glb` 生成までは成立したが、multi-frame final quality は未了のため全体は `active` に残す。現時点の主 blocker は `mRL-10.4` であり、実 artifact では chunk 間 global 接続のぶれが残る。したがって方針を `ARCore sequence anchor を baseline / fallback / judge に残しつつ、DA3 NGL 推定 camera trajectory を merge 主座標へ使う experimental route を追加比較する` へ更新し、優先順は `route 比較設計固定`、`pred_extrinsics` の pose convention 固定、positive similarity 強制、PLY で camera / scene 一致確認、GLB scene graph 対応、最後に owner_record keep 微調整とする |
...
| `BLK-4` | `P4` | `adjacent` | `Colab` runbook の canonical route がまだ旧 proof 設計の残骸を引きずっており、sequence-anchor first、adjacent continuity precheck、official API 実行、pre-merge gate が文書・pair・app 既定値で完全一致していない。加えて authoring 面では `repo bootstrap`、helper、wrapper source の notebook 埋め込みが強く、局所修正で閉じにくい | `1 record` 化した data を `DA3` へ渡す前に `Colab` 側で整合を壊し、`DA3NESTED-GIANT-LARGE-1.1` canonical route の本番品質を落とす。runbook 自体の保守性も下がる | `mRL-10.1` と `mRL-10.2` は `p-done` として前進したが、runbook の `.md/.ipynb` pair と admin closeout は継続更新する。`mRL-10.5` では設計契約書、source-sync、HAUB の関数 / 変数一覧まで含めて固定する。関連 gate は `MRL-10`,`mRL-10.1`,`mRL-10.2`,`mRL-10.5` | `small-open` | `hi-ai-unified-blueprint.md`, `da3_ngl_prepose_RB.md`, `C:/Users/tetsuya/kisaragi/kisaragi-db/--devs/--products/prj-kisaragi_0002/colab/da3_ngl_runbook_design_contract.md` |
...
| `td14a` | `bd16a` | Drive input candidate scan and select | runbook が Drive 上の session zip / session folder 候補を列挙し、selected input を固定して、後続 `Step 1` と `MRL-7` one-block が hardcoded path なしで同じ入力を読める | pass | `kisaragi-db/--devs/--products/prj-kisaragi_0002/colab/da3_ngl_prepose_RB.md` |
...
旧 `DA3 script 一覧表` を `modeling` 用一覧表として扱う。`camera trajectory`、`pose`、`intrinsics`、`anchor basis` の共通契約は `#6 Shared Helpers` に集約し、runbook の基本順は `#6 -> #9 -> #10 -> #11 -> #7 -> #8 -> #13 -> #14` とする。`#7 Full Prepose Build` は anchor stage として chunk-local `DA3 NGL` pose build、overlap matching、chunk graph build、可視化確認までを内包し、`#13` は build 済み prepose graph を review する thin gate、`#14` はその graph を消費する merge engine として扱う。

| token | source_file | role | key functions / classes | key data names | reference directories | main outputs / handoff | docs_id |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `#1-1` | `cells/01_01.py` | runbook title と route 宣言 | - | - | `colab/` | canonical pair 冒頭定義 | `DOC-C01-01` |
| `#2-1` | `cells/02_01.py` | config 固定 | - | `CONFIG` | `/content`, `probe_root` | target chunk policy、reset policy、route config | `DOC-C02-01` |
| `#3-1` | `cells/03_01.py` | input select と path 解決 | `infer_session_id`, `scan_candidates`, `extract_selected_input`, `resolve_and_validate_paths` | `RUNBOOK_SELECTED_DOC`, `RUNBOOK_PATHS_DOC`, `CONFIG_SNAPSHOT` | `Google Drive input`, `RESULTS_ROOT`, `EXTRACT_ROOT` | `runbook_selected_input.json`, `runbook_paths.json` | `DOC-C03-01` |
| `#4-1` | `cells/04_01.py` | fresh run tree init と reset | - | `paths`, `probe_root`, `pipeline_root`, `frame_record_path` | `results_root`, `probe_root`, `session_root` | session context と fresh tree | `DOC-C04-01` |
| `#4-2` | `cells/04_02.py` | managed dir registry 生成 | - | `ctx`, `managed_dirs` | `probe_root`, `01_anchor`, `manifests`, `final_outputs` | `managed_dirs` contract | `DOC-C04-02` |
| `#5-1` | `cells/05_install.py` | repo / runtime install | - | `repo_root`, `repo_url`, `src_root` | `repo_root`, `src_root` | local source bootstrap | `DOC-C05-01` |
| `#6-1` | `cells/06_shared_helpers.py` | shared helper 契約 | `load_ctx`, `save_json`, `append_sequence_columns`, `display_stage_summary` | `RUNBOOK_CTX_PATH` | `/content`, `probe_root` | shared context / summary API | `DOC-C06-01` |
| `#7-1` | `cells/07_01.py` | frame record から anchor 入力を作る | `build_anchor_inputs_from_zip` | `ctx`, `manifest_dir`, `images_dir`, `frame_record_path` | `images_dir`, `manifest_dir`, `da3_nested_dir`, `world_dir` | anchor build 入力一式 | `DOC-C07-01` |
| `#7-2` | `cells/07_02.py` | full anchor artifact を構築する | `_existing`, `_find_manifest_triplet`, `_normalize_rows` | `manifest_df`, `intrinsics`, `extrinsics_w2c`, `camera_anchor_full_df`, `camera_matrix_full_csv` | `persist_root`, `manifest_dir`, `01_anchor` | `camera_anchor_full_arc.csv`, `camera_matrix_full_arc.csv`, `camera_center_matrix_arc.csv`, `camera_orientation_full_arc.csv`, `intrinsics.npy`, `extrinsics_w2c_arc.npy`, `c2w_arc.npy` | `DOC-C07-02` |
| `#7-3` | `cells/07_03.py` | full anchor pose diag を作る | `_find_anchor_root`, `_normalize`, `_wrap_deg`, `_angle_deg` | `anchor_df`, `yaw_deg`, `pitch_deg`, `roll_deg`, `delta_pos` | `persist_root`, `01_anchor` | `full_anchor_pose_diag_arc.csv` | `DOC-C07-03` |
| `#7-6` | `cells/07matching_01.py` | overlap pose matching | `resolve_matching_chunk_names`, `resolve_chunk_artifact`, `estimate_pose_aware_similarity`, `transform_c2w_list`, `plot_pose_match`, `write_pose_match_html` | `MATCHING_CHUNK_IDS_1BASED`, `chunk_a_name`, `chunk_b_name`, `overlap_records`, `relative_rotation_deg`, `summary` | `persist_root/01_anchor/07matching`, `final_outputs/chunk_evidence`, `pipeline_root/chunk_runs` | `*_overlap_pair_metrics_arc.csv`, `*_trajectory_points_arc.csv`, `*_transform_b_to_a.npy`, `*_trajectory_match.png`, `*_trajectory_match.html`, `*_matching_summary.json` | `DOC-C07M-01` |
| `#8-1` | `cells/08_01.py` | anchor QC | `_find_anchor_diag_root` | `df`, `roll_deg_centered`, `fail_df`, `warn_df`, `summary` | `persist_root`, `01_anchor` | `full_anchor_pose_qc_arc.csv`, fail / warn CSV | `DOC-C08-01` |
| `#8-2` | `cells/08_02.py` | anchor 可視化 | `_find_anchor_root`, `_resolve_center_cols`, `_resolve_lens_cols` | `anchor_csv`, `centers`, `plotly_html`, `plotly_png` | `persist_root`, `01_anchor` | anchor trajectory HTML / PNG | `DOC-C08-02` |
| `#9-1` | `cells/09_01.py` | record manifest 正規化 | `ranked_image_dirs`, `lap_var`, `read_actual_wh`, `normalize_intrinsics_to_upright`, `quat_to_rot`, `pose_to_w2c`, `build_K` | `frame_pose_df`, `image_name_by_record_index`, `rows` | `images_dir`, `frame_record_path`, `frame_pose_index_path`, `manifest_dir` | `da3_input_manifest.csv`, `intrinsics.npy`, `extrinsics_w2c_arc.npy` | `DOC-C09-01` |
| `#9-2` | `cells/09_02.py` | record と anchor diag の join | - | `anchor_diag`, `anchor_join`, `df` | `manifest_dir`, `01_anchor` | joined manifest 補助表 | `DOC-C09-02` |
| `#9-3` | `cells/09_03.py` | config view 固定 | - | `config`, `config_view` | `pipeline_root` | config summary 表示 | `DOC-C09-03` |
| `#9-4` | `cells/09_04.py` | chunk plan と batch plan の canonical 生成 | `sha256_file`, `to_4x4` | `pipeline_root`, `anchor_dir`, `chunk_manifest_dir`, `batch_runs_dir`, `merged_dir`, `chunk_index_all`, `chunk_index_target`, `batch_plan_df`, `chunk_sequence_anchor_index_df` | `persist_root`, `01_anchor`, `pipeline_root/manifests`, `pipeline_root/04_batch_runs`, `pipeline_root/05_merge` | `pipeline_config.json`, `chunk_index_all.csv`, `chunk_index_target.csv`, `chunk_sequence_anchor_index.csv`, `batch_plan.csv`, `batch_plan_summary.json` | `DOC-C09-04` |
| `#9-5` | `cells/09_05.py` | record / chunk summary 表示 | - | `record_df`, `chunk_index_df`, `out` | `pipeline_root/manifests` | summary table | `DOC-C09-05` |
| `#10-1` | `cells/10_01.py` | chunk precheck table | - | `chunk_index_df`, `precheck_df` | `pipeline_root/manifests` | `chunk_precheck_arc.csv` | `DOC-C10-01` |
| `#10-2` | `cells/10_02.py` | adjacent edge validation | - | `idx_df`, `edge_df` | `pipeline_root/manifests` | `adjacent_edge_validation_arc.csv` | `DOC-C10-02` |
| `#10-3` | `cells/10_03.py` | precheck gate summary | - | `anchor_qc_df`, `sequence_df`, `edge_df` | `persist_root/01_anchor`, `pipeline_root/manifests` | precheck gate summary JSON / CSV | `DOC-C10-03` |
| `#11-1` | `cells/11_01.py` | batch overview | - | `batch_plan_df`, `chunk_all_df`, `chunk_target_df` | `manifest_dir`, `pipeline_root/manifests` | batch overview | `DOC-C11-01` |
| `#11-2` | `cells/11_02.py` | preflight utility 群 | `load_json`, `save_json`, `find_first`, `normalize_rows`, `angle_deg` | - | `/content`, `probe_root` | utility-only | `DOC-C11-02` |
| `#11-3` | `cells/11_03.py` | execution target 固定 | - | `execution_chunks_df`, `execution_batch_plan_path`, `canonical_chunk_with_batch_path` | `pipeline_root/manifests`, `pipeline_root/chunk_runs` | `execution_target_chunks.csv`, `execution_target_batch_plan.csv` | `DOC-C11-03` |
| `#11-4` | `cells/11_04.py` | run preflight | - | `required_paths`, `record_df`, `anchor_pose_df`, `preflight` | `manifest_dir`, `persist_root/01_anchor`, `pipeline_root/manifests`, `final_outputs` | preflight summary、存在確認 | `DOC-C11-04` |
| `#11-5` | `cells/12_01.py` | batch execution item 生成 | - | `execution_chunks_df`, `execution_batch_plan_df`, `batch_execution_items_df` | `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `final_outputs/diagnostics` | `batch_execution_items.csv`, `batch_execution_items_summary.json`, `/content/runbook_batch_preflight_status.json` | `DOC-C12-01` |
| `#7-4` | `cells/12_chunk_wrapper.py` | chunk 実行 wrapper | `_resolve_device`, `_pick_pred_array`, `main` | `REPO_ROOT`, `SRC_ROOT`, `export_format`, `gs_ply_saved` | `repo_root`, `src_root`, `chunk_runs_dir` | chunk ごとの `pred_extrinsics.npy`, `pred_intrinsics.npy`, `gs_ply/0000.ply`, `_SUCCESS.json` | `DOC-C12-02` |
| `#7-5` | `cells/12_03.py` | chunk-local DA3 prepose build | - | `wrapper_path`, `items_df`, `DRY_RUN`, `DEVICE`, `EXPORT_FORMAT` | `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `final_outputs/diagnostics` | run manifest、batch launch summary、chunk local pose / GS | `DOC-C12-03` |
| `#7-7` | `cells/07_07.py` | prepose graph build and gate | `to_4x4_batch`, `build_anchor_c2w_list`, `local_c2w_list`, `estimate_pose_aware_similarity`, `transform_c2w_list`, `pick_selected_candidate`, `summarize_relative_transform` | `anchor_dir`, `camera_anchor_full_path`, `LOCAL_EXTRINSIC_MODE`, `LOCAL_CAMERA_BASIS`, `route_label`, `fallback_used`, `candidate_df`, `validation_df`, `graph_solution_rows`, `graph_edge_rows`, `summary`, `relative_rotation_deg` | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/diagnostics` | `pred_vs_anchor_pose_residual.csv`, `premerge_route_compare_arc.csv`, `premerge_route_compare_summary.json`, `premerge_pose_validation.csv`, `premerge_pose_validation.json`, `prepose_chunk_graph_solution_arc.csv`, `prepose_chunk_graph_edges_arc.csv`, `prepose_chunk_graph_summary.json` | `DOC-C07-07` |
| `#13-1` | `cells/13_01.py` | prepose graph review gate | - | `validation`, `route_compare_summary`, `graph_summary`, `review_summary`, `preferred_fallback_used_count` | `pipeline_root/manifests`, `pipeline_root/merged`, `final_outputs/diagnostics` | `prepose_graph_gate_review.json` と build artifact review | `DOC-C13-01` |
| `#14-1` | `cells/14_01.py` | merge from prepose graph | `ensure_target_chunk_manifest`, `resolve_chunk_input_dir`, `select_route_candidate`, `summarize_relative_transform` | `persist_root`, `anchor_dir`, `global_anchor_df`, `global_camera_matrix_df`, `LOCAL_EXTRINSIC_MODE`, `LOCAL_CAMERA_BASIS`, `route_label`, `fallback_used`, `prepose_graph_solution_by_chunk`, `chunk_global_transforms_arc`, `keep_df`, `merge_summary`, `relative_rotation_deg` | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/*` | `merged_gs_arc.ply`, `merged_scene_arc.glb`, `chunk_global_transforms_arc.csv`, `merge_route_compare_arc.csv`, `prepose_chunk_graph_solution_arc.csv`, `chunk_keep_summary_arc.csv`, `chunk_transform_quality_arc.csv`, `owner_record_histogram_arc.csv`, `chunk_assignment_summary_arc.csv`, `merge_summary.json`, `final_output_manifest_arc.json` | `DOC-C14-01` |
| `#15-1` | `cells/15_01.py` | optional local bundle | - | `merge_summary`, `bundle_model_slug`, `local_bundle_zip`, `bundle_download_summary` | `pipeline_root/merged`, `probe_root` | local bundle zip と summary | `DOC-C15-01` |
| `#16-1` | `cells/16_01.py` | cleanup inventory | `path_size_bytes` | `persist_root`, `anchor_dir`, `kept_groups`, `delete_candidates`, `cleanup_plan` | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/*` | `cleanup_plan.json` | `DOC-C16-01` |
| `#16-2` | `cells/16_02.py` | cleanup review table | - | `cleanup_plan`, `rows`, `cleanup_csv` | `pipeline_root/merged` | cleanup review CSV | `DOC-C16-02` |
| `#17-1` | `cells/17_01.py` | cleanup apply | - | `cleanup_plan`, `review_df`, `deleted`, `kept`, `cleanup_result` | `pipeline_root/merged`, `chunk_runs_dir`, local tmp paths | cleanup result JSON | `DOC-C17-01` |

#### modeling用 runbook設計契約表

`HAUB` では、runbook の設計契約と source 管理の要点もこの直後の表で保持する。詳細本文は `da3_ngl_runbook_design_contract.md` と `da3_ngl_runbook_source_inventory.md` に置くが、日常運用でまず見る面はこの節とする。

| 区分 | 対象 stage / file | 固定したいこと | 主な data / directory | judge / output |
| --- | --- | --- | --- | --- |
| 設計契約 | `#6 Shared Helpers` | camera trajectory / pose / intrinsics / anchor basis の唯一の共通契約面にする | `/content`, `probe_root`, `persist_root/01_anchor`, `pipeline_root/manifests` | helper 契約の再利用、summary 表示 |
| matching 契約 | `#7-6` | 2 chunk overlap pose を同じ座標系へ事前整合し、後段 graph / merge の前に scale、rotation、translation、pre/post residual を診断できるようにする。input 不足時は `skipped` で返し、top-to-bottom 実行を壊さない | `persist_root/01_anchor/07matching`, `final_outputs/chunk_evidence`, `pipeline_root/chunk_runs`, `MATCHING_CHUNK_*`, `relative_rotation_deg` | `*_matching_summary.json`, `*_overlap_pair_metrics_arc.csv`, `*_trajectory_points_arc.csv`, `*_trajectory_match.png`, `*_trajectory_match.html`, `*_transform_b_to_a.npy` |
| build 契約 | `#7-7` | baseline route を fallback / judge 面に残し、`DA3 predicted trajectory experimental route` を primary 候補として比較する。比較結果を route 選択だけで終わらせず、`chunk_to_world` の事前整合 graph として固定し、chunk ごとの `route_label`、`fallback_used`、overlap / non-overlap residual、chunk 間 relative transform を見えるようにする | `persist_root/01_anchor`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `route_label`, `fallback_used`, `graph_parent_chunk_name`, `t00..t33`, `relative_rotation_deg` | `premerge_pose_validation.json`, `premerge_route_compare_summary.json`, `prepose_chunk_graph_solution_arc.csv`, `prepose_chunk_graph_edges_arc.csv`, residual / gate CSV |
| review 契約 | `#13-1` | `#7-7` で build 済みの route compare と graph artifact を review し、`#14-1` に渡す thin gate とする | `pipeline_root/merged`, `final_outputs/diagnostics`, `preferred_route_label`, `preferred_fallback_used_count` | `prepose_graph_gate_review.json`, review summary |
| merge 契約 | `#14-1` | `#13-1` で確定した prepose graph を優先消費する merge engine にし、必要時だけ route solve を互換 fallback として使う。採用 route と fallback 発生を merged diagnostics に残し、`fallback_used` に加えて `preferred_fallback_used` を残して judge 済み baseline 採用と preferred route からの実 fallback を区別して読めるようにする。chunk 間 relative transform も merge diagnostics に残す | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/*`, `route_label`, `fallback_used`, `prepose_graph_solution_by_chunk`, `relative_rotation_deg` | `merge_summary.json`, `merge_route_compare_arc.csv`, `prepose_chunk_graph_solution_arc.csv`, `chunk_global_transforms_arc.csv`, `chunk_transform_quality_arc.csv`, merged GS / GLB |
| authoring 契約 | `da3_runbook_sources/` | `.md/.ipynb` pair は source-sync 前提で管理し、cell / markdown を同じ task で同期する | `da3_runbook_sources/cells`, `da3_runbook_sources/markdown`, `cell_manifest.json`, `markdown_manifest.json` | pair 再生成、inventory 再生成 |
| trace 契約 | `HAUB` と inventory | role、key data names、reference directories、main outputs / handoff を 1 表で追えるようにする | `hi-ai-unified-blueprint.md`, `da3_ngl_runbook_source_inventory.md` | 参照面の取り違え防止 |
...
| `da3_ngl_runbook_source_inventory.md` | 補助詳細 | cell / markdown / function / variable の台帳を持つ | どの source を触るか、docs_id と対応を追う時 |
...
| `MRL-10` | `-` | `frame_record.jsonl + images` を正とする record-native `Colab` route を canonical 化し、`sequence-anchor first`、`1 frame = 1 record`、`strict one-to-one join`、`adjacent continuity precheck`、`official API / CLI` 基準を [da3_ngl_prepose_RB.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.md) / [da3_ngl_prepose_RB.ipynb](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.ipynb) の canonical pair へ固定する。camera pose / trajectory の事前推定も `DA3NESTED-GIANT-LARGE-1.1` だけで行い、`mRL-10.1` と `mRL-10.2` は `p-done`、残りは chunk route、merge 品質、runbook 構成固定である | `sd9`,`su12`,<br>`su14`,`sd11` | `bd16`,`bd17`,<br>`bd18a`,`bd20` | `td16`,`td17`,<br>`td18`,`td19a`,<br>`td23` | `active` | `active` | [da3_ngl_prepose_RB.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.md) の<br>`#1`-`#17` | `未収載` |
| `MRL-10` | `mRL-10.1` | `frame_record.jsonl`、対応 image 群、tracking / K / pose / timestamp を正に読んで、`input_frame_manifest.csv`、`input_frame_qc.csv`、`pose_conversion_check.csv`、`k_resize_check.csv`、`orientation_summary.json`、`da3_input_manifest_*.csv` を生成できる | `sd9`,`sd11` | `bd16`,`bd17`,<br>`bd20` | `td16`,`td23` | `p-done` | `p-done` | [da3_ngl_prepose_RB.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.md) の<br>`#7`-`#8` | `未収載` |
| `MRL-10` | `mRL-10.2` | proof / production を分離した sequence-anchor route が image-only 推論で通り、production 側では `frame_record` 由来の `intrinsics` / `extrinsics_w2c` を使って world point cloud と評価 summary まで保存できる。upright canonical image と legacy intrinsics 補正結果が同じ manifest 群で残る | `sd9`,`su14`,<br>`sd11` | `bd17`,`bd18`,<br>`bd20` | `td17`,`td18`,<br>`td23` | `p-done` | `p-done` | [da3_ngl_prepose_RB.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.md) の<br>`#9`-`#10` | `未収載` |
| `MRL-10` | `mRL-10.3` | `Giant` production candidate route は `continuous_gs_v06_chunk18_overlap6_adopt12` とし、canonical runbook の chunk planning section で `extrinsics_w2c_prod.npy` から全 frame の global camera matrix、`camera_matrix_full_arc.csv`、全 chunk manifest、`batch_plan.csv` を作る。`chunk_size = 18`、`step = 12`、`adopt_size = 12`、`chunks_per_batch = 3`、`process_res = 504` を config 正本で固定し、各 chunk は `6frame` 重複しつつ基本 `後半 12frame` を再構成責務とする。chunk 構築と batch 実行自体は成立したが、final merge 品質は `mRL-10.4` 未了のため全体は `active` とする | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | [da3_ngl_prepose_RB.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.md) の<br>`#11`-`#12` | `未収載` |
| `MRL-10` | `mRL-10.4` | chunk merge は `ARCore anchor baseline route` と `DA3 NGL predicted trajectory experimental route` の 2 本を比較できる設計へ改める。`ARCore` は fallback / judge と residual 計測の基準に残し、`DA3` 側は merge 主座標候補として扱う。現時点の主課題は owner keep 調整より前に、この route 比較設計、`pred_extrinsics` の pose convention 固定、positive similarity 強制、PLY / GLB 一致確認を閉じることである。その後に `owner_record_index` keep を再調整し、`chunk_global_transforms_arc.csv`、`vertex_assignment_summary.csv`、`owner_record_histogram_arc.csv`、`chunk_assignment_summary_arc.csv`、`merge_warning_summary_arc.json`、`chunk_transform_quality_arc.csv` を残す | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | [da3_ngl_prepose_RB.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.md) の<br>`#13`-`#15` | `未収載` |
| `MRL-10` | `mRL-10.5` | canonical runbook pair 自体の設計を固定する。対象は [da3_ngl_prepose_RB.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.md) / [da3_ngl_prepose_RB.ipynb](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.ipynb) であり、`#1`-`#17` の clean bootstrap 構成、section と code cell 番号一致、official API / CLI 基準、`extrinsics_w2c_arc.npy` provenance 明示、anchor 以降の各出力に対する input / output summary 表示、admin 手順と artifact 契約の同期、さらに source file から pair を同期できる authoring 面と `HAUB` 上の関数 / 変数一覧を維持する | `sd9`,`su12`,<br>`sd11` | `bd17`,`bd20` | `td16`,`td17`,<br>`td23` | `active` | `active` | [da3_ngl_prepose_RB.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_prepose_RB.md) の<br>`#1`-`#17`、[da3_ngl_runbook_design_contract.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook_design_contract.md)、admin 手順 38-45 | `未収載` |

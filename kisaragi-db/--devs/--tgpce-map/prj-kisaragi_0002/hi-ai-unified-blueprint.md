...
| `#4-1..#4-2` | `cells/04_01.py`, `cells/04_02.py` | tree init と managed dir 宣言 | - | `probe_root`, `pipeline_root`, `managed_dirs` | `results_root`, `probe_root`, `session_root` | session context、managed dir contract | `DOC-I04-01`,`DOC-I04-02` |
...
| `#8-1..#8-9` | `cells/08_01.py`-`cells/08_09.py` | record manifest 正規化、chunk plan、incremental chunk 実行 | `ranked_image_dirs`, `normalize_intrinsics_to_upright`, `apply_incremental_seed_to_chunk_df`, `update_accepted_pose_map_from_chunk` | `da3_input_manifest.csv`, `batch_execution_items.csv`, `batch_run_status_arc.csv`, `incremental_seed_trace_arc.csv` | `images_dir`, `frame_record_path`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `final_outputs/diagnostics` | chunk 実行証跡、predicted pose artifact | `DOC-I08-01`-`DOC-I08-09` |
| `#9-1` | `cells/09_01.py` | overlap matching | `resolve_matching_chunk_names`, `resolve_chunk_artifact`, `plot_pose_match`, `write_pose_match_html` | `MATCHING_CHUNK_IDS_1BASED`, `relative_rotation_deg`, `summary` | `persist_root/01_anchor/07matching`, `final_outputs/chunk_evidence`, `pipeline_root/chunk_runs` | matching CSV / PNG / HTML / summary | `DOC-I09-01` |
| `#10-1..#10-2` | `cells/10_01.py`, `cells/10_02.py` | global graph build / gate / review | `_load_chunk_pose_df`, `_poses_to_center_lens` ほか | `premerge_pose_validation.json`, `prepose_chunk_graph_solution_arc.csv`, `chunk_global_transforms_arc.csv`, `prepose_chunk_graph_summary.json` | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged` | graph solution、validation、review summary | `DOC-I10-01`,`DOC-I10-02` |
| `#11-1..#11-2` | `cells/11_01.py`, `cells/11_02.py` | merge / review visualization | `ensure_target_chunk_manifest`, `_load_transform_map`, `_poses_to_centers_dirs` | `merged_camera_pose_arc.csv`, `merged_camera_matrix_arc.csv`, `ngl_bundle_manifest_dir`, `merge_summary.json` | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/*` | merged GS / GLB、NGL bundle、review HTML | `DOC-I11-01`,`DOC-I11-02` |
...
| incremental 契約 | `#8-1..#8-9` | `sliding_window_incremental_seeded` として、前 chunk の adopted pose を次 chunk の context へ seed しながら chunk を漸次実行する。seed 面を失わないよう `batch_run_status_arc.csv` と `incremental_seed_trace_arc.csv` を同時に残す | `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `final_outputs/diagnostics`, `POSE_PIPELINE_MODE`, `seed_pose_by_record_index` | `batch_run_status_arc.csv`, `incremental_seed_trace_arc.csv`, `all_batch_summary_arc.json` |
| matching 契約 | `#9-1` | 2 chunk overlap pose を同じ座標系へ事前整合し、graph / merge の前に scale、rotation、translation、pre/post residual を診断できるようにする。input 不足時は `skipped` で返し、top-to-bottom 実行を壊さない | `persist_root/01_anchor/07matching`, `final_outputs/chunk_evidence`, `pipeline_root/chunk_runs`, `MATCHING_CHUNK_*`, `relative_rotation_deg` | `*_matching_summary.json`, `*_overlap_pair_metrics_arc.csv`, `*_trajectory_points_arc.csv`, `*_trajectory_match.png`, `*_trajectory_match.html`, `*_transform_b_to_a.npy` |
| build 契約 | `#10-1` | predicted pose と anchor を graph へ上げ、chunk ごとの world transform と merge 前 validation を固定する。`chunk_global_transforms_arc.csv` 互換の matrix contract を維持しつつ、`prepose_chunk_graph_solution_arc.csv` と `premerge_pose_validation.json` を残す | `persist_root/01_anchor`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `route_label`, `center_error_p95`, `delta_center_error_max`, `t00..t33` | `premerge_pose_validation.json`, `prepose_chunk_graph_solution_arc.csv`, `prepose_chunk_graph_summary.json`, `chunk_global_transforms_arc.csv` |
| review 契約 | `#10-2` | `#10-1` の graph artifact を review summary へ整え、merge 前判断面を作る | `pipeline_root/merged`, `final_outputs/diagnostics`, `graph_summary`, `review_summary` | review summary JSON |
| merge 契約 | `#11-1..#11-2` | graph で確定した chunk-to-world を使って merge し、merged pose / NGL bundle / GS artifact と review visualization を一体で残す | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/*` | `merge_summary.json`, `merged_camera_pose_arc.csv`, `merged_camera_matrix_arc.csv`, merged GS / GLB、review HTML |
...
| `PATH-I03` | `batch_execution_items.csv` | `#8-3,#8-7` | `#8-4,#8-5,#8-9,#10-1,#11-2` | `pipeline_root/manifests/batch_execution_items.csv` | canonical chunk execution item manifest。`batch_work_dir` は batch scope、`chunk_out_dir` は chunk scope |
| `PATH-I04` | `chunk_input_manifest_arc.csv` | `#8-3` | `#8-4` | `pipeline_root/manifests/chunk_input_manifest_arc.csv` | chunk local / context / adopt manifest |
| `PATH-I05` | `chunk_index_all.csv` | `#8-3` | `#8-4,#8-5,#11-1` | `pipeline_root/manifests/chunk_index_all.csv` | chunk 全体 index と backward 互換 |
| `PATH-I06` | `chunk_index_target.csv` | `#8-5,#11-1` | `#8-5,#11-1` | `pipeline_root/manifests/chunk_index_target.csv` | target subset self-heal manifest |
| `PATH-I07` | `batch_plan.csv` | `#8-5` | `#8-5` | `pipeline_root/manifests/batch_plan.csv` | target batch grouping self-heal manifest |
| `PATH-I08` | `execution_target_chunks.csv` | `#8-5` | `#8-6,#8-7` | `pipeline_root/manifests/execution_target_chunks.csv` | execution 用 target chunks |
| `PATH-I09` | `execution_target_batch_plan.csv` | `#8-5` | `#8-6,#8-7` | `pipeline_root/manifests/execution_target_batch_plan.csv` | execution 用 target batch plan |
| `PATH-I10` | `pred_extrinsics.npy` | `#8-8` | `#8-9,#9-1,#10-1,#11-1,#11-2` | `pipeline_root/chunk_runs/<batch_name>/<chunk_name>/pred_extrinsics.npy` | chunk 単位 pose artifact。reader は `chunk_out_dir` 優先、legacy suffix dir fallback を許容 |
| `PATH-I11` | `chunk_input_frames.csv` | `#8-8` | `#9-1,#11-1` | `pipeline_root/chunk_runs/<batch_name>/<chunk_name>/chunk_input_frames.csv` | chunk 実行時に使った frame manifest。`chunk_input_frames*.csv` と seeded runtime csv の legacy fallback を許容 |
...
| `PATH-I14` | `premerge_pose_validation.json` | `#10-1` | `#10-2,#11-1` | `pipeline_root/merged/premerge_pose_validation.json` | merge 前 gate |
| `PATH-I15` | `prepose_chunk_graph_solution_arc.csv` | `#10-1` | `#10-2,#11-1,#11-2` | `pipeline_root/merged/prepose_chunk_graph_solution_arc.csv` | chunk-to-world graph solution |
| `PATH-I16` | `chunk_global_transforms_arc.csv` | `#10-1` | `#11-2` | `pipeline_root/manifests/chunk_global_transforms_arc.csv` | downstream 互換 transform |
| `PATH-I17` | `merged_camera_pose_arc.csv` | `#11-1` | `#11-2` | `pipeline_root/merged/merged_camera_pose_arc.csv` | merged pose review input |

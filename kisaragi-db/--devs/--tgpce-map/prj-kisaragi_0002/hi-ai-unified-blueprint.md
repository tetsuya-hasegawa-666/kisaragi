...
| `modeling` | `MRL-3` から `MRL-9` は `p-done` である。`MRL-10` では、`mRL-10.1` と `mRL-10.2` は `frame_record.jsonl + images` を正にした record-native input 正規化、QC、proof / production 分離、world point cloud 保存まで到達しており `p-done` とみなす。`mRL-10.3` は `continuous_gs_v06_chunk18_overlap6_adopt12` の `global camera matrix -> camera_matrix_full_arc.csv -> chunk manifest -> batch_plan -> 3chunk batch 実行` までは成立したが、final rebuild merge 品質は未了のため全体は `active` に残す。現時点の主 blocker は `mRL-10.4` であり、実 artifact では chunk ごとの `scale` が負または極小になりやすく、camera pose と scene の global 向きが一致しない。したがって優先順は `pred_extrinsics` の pose convention 固定、positive similarity 強制、PLY で camera / scene 一致確認、GLB scene graph 対応、最後に owner_record keep 微調整である |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `BLK-1` | `P1` | `direct` | `continuous_gs_v06_chunk18_overlap6_adopt12` の merge は、現 artifact では chunk ごとの `scale` が負または極小になりやすく、camera pose と scene の global 向きが一致しない。owner keep 調整より前に pose convention と similarity 拘束を固定する必要がある | `merged_gs_arc.ply` / `merged_scene_arc.glb` が意図した camera pose に乗らず、chunk 境界の混線や二重像が残る。PLY と GLB の両方が壊れるため merge 後段だけでは閉じない | 優先順を `mRL-10.4c pose convention 固定` -> `mRL-10.4d positive similarity と hard fail` -> `mRL-10.4e owner_record keep 再調整` -> `mRL-10.4f GLB scene graph 対応` -> `mRL-10.4g admin viewer 再確認` へ更新する。関連 gate は `MRL-10`,`mRL-10.3`,`mRL-10.4`,`mRL-10.4c`-`mRL-10.4g` | `big-open` | `hi-ai-unified-blueprint.md`, `da3_ngl_runbook.md`, `C:/Users/tetsuya/kisaragi/kisaragi-db/--exsams/prj-kisaragi_0002/colab-outputs/trajectreview-modeling-session-20260403_gl11_c18ov6ad12/continuous_gs_v06_chunk18_overlap6_adopt12/merged/chunk_transform_quality_arc.csv` |
...
1. `MRL-10 / mRL-10.4c` として、代表 chunk で `pred_extrinsics.npy` の pose convention を固定し、`w2c` / `c2w` と必要な軸反転候補のうち、正 `scale` で global camera matrix に最も整合する解釈を決める
2. `MRL-10 / mRL-10.4d` として、similarity を positive 側へ拘束し、`scale <= 0`、極小 `scale`、`center_rmse` 超過、`rotation_dir_residual` 超過を hard fail にする
3. `MRL-10 / mRL-10.4e` として、PLY で camera / scene 一致を確認した上で owner_record keep を再調整し、後段 merge quality を切り分ける
4. `MRL-10 / mRL-10.4f` として、GLB scene graph の node transform を保持したまま global transform を適用し、PLY と GLB の差分要因をつぶす
5. `MRL-2R` として、canonical input を採択 frame record へ切り替え、recording runtime、`Sampling条件` popup、transfer 契約、parser / modeling script、旧 thinning logic 削除を 1 本の計画でそろえる
6. `MRL-2S` を redesigned runtime 上で再実施し、`correcting` の bounded stop 修正 build を `3min` 実収録で確認して連続収録安定化を閉じる
7. `MRL-7` を `TraceCore` の最小表示に限定し、route 比較と viewer 後続論点を `MRL-**` へ切り分ける
8. `MRL-10` の後で、`gs_ply` を top camera renderer と reviewing viewer へ接続する route、multi-view / 長時間 optimization、request / status / result download UX を整理する
...
| `td14a` | `bd16a` | Drive input candidate scan and select | runbook が Drive 上の session zip / session folder 候補を列挙し、selected input を固定して、後続 `Step 1` と `MRL-7` one-block が hardcoded path なしで同じ入力を読める | pass | `kisaragi-db/--devs/--products/prj-kisaragi_0002/colab/da3_ngl_runbook.md` |
...
| `#12-2` | `cells/12_chunk_wrapper.py` | chunk 実行 wrapper | `_resolve_device`, `_pick_pred_array`, `main` | `REPO_ROOT`, `SRC_ROOT` | `repo_root`, `src_root`, `chunk_runs_dir` | chunk ごとの `pred_extrinsics.npy`, `pred_intrinsics.npy`, `_SUCCESS.json` | `DOC-C12-02` |
| `#12-3` | `cells/12_03.py` | batch launch | - | `wrapper_path`, `items_df`, `DRY_RUN`, `DEVICE` | `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `final_outputs/diagnostics` | run manifest、batch launch summary | `DOC-C12-03` |
| `#13-1` | `cells/13_01.py` | pre-merge pose gate | `resolve_center_cols`, `resolve_ypr_cols`, `resolve_lens_cols`, `lens_from_yaw_pitch_deg` | `residual_df`, `gate_df`, `validation_df`, `summary` | `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/diagnostics` | `pred_vs_anchor_pose_residual.csv`, `premerge_pose_gate.csv`, `premerge_pose_validation.csv`, `premerge_pose_validation.json` | `DOC-C13-01` |
| `#13-2` | `cells/13_02.py` | pose convention probe | `to_4x4_batch`, `normalize_rows`, `angle_deg`, `rotate_about_axis`, `build_anchor_c2w_list`, `basis_matrix`, `local_c2w_candidates`, `estimate_pose_aware_similarity` | `premerge_pose_validation`, `failed_chunk_names`, `basis_candidates`, `candidate_df` | `pipeline_root/manifests`, `pipeline_root/merged`, `final_outputs/diagnostics` | `premerge_pose_probe_candidates.csv`, `premerge_pose_probe_summary.json` | `DOC-C13-02` |
| `#13-3` | `cells/13_03.py` | center / forward / up split probe | `to_4x4_batch`, `normalize_rows`, `angle_deg`, `build_anchor_arrays`, `basis_matrix`, `local_arrays` | `failed_chunk_names`, `basis_candidates`, `split_df`, `best_df` | `pipeline_root/manifests`, `pipeline_root/merged`, `final_outputs/diagnostics` | `premerge_pose_probe_split_metrics.csv`, `premerge_pose_probe_split_summary.json` | `DOC-C13-03` |
| `#13-4` | `cells/13_04.py` | raw orientation inspection | `normalize_rows`, `to_4x4_batch`, `basis_matrix`, `resolve_anchor_arrays`, `local_arrays` | `failed_chunk_names`, `basis_candidates`, `inspection_df` | `pipeline_root/manifests`, `pipeline_root/merged`, `final_outputs/diagnostics` | `premerge_pose_raw_orientation_inspection.csv`, summary JSON | `DOC-C13-04` |
| `#13-5` | `cells/13_05.py` | ARCore full anchor 妥当性チェック | `normalize_rows`, `angle_deg` | `persist_root`, `anchor_dir`, `camera_anchor_full_path`, `anchor_df`, `centers`, `lens`, `up`, `valid_df` | `persist_root/01_anchor`, `pipeline_root/merged`, `final_outputs/diagnostics` | `arcore_anchor_trajectory_validation.csv`, `arcore_anchor_trajectory_validation_summary.json` | `DOC-C13-05` |
| `#13-6` | `cells/13_06.py` | failed chunk と full anchor の join-ready 準備 | - | `persist_root`, `anchor_dir`, `camera_anchor_full_path`, `camera_matrix_full_path`, `anchor_full_df`, `matrix_full_df`, `join_df` | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/merged`, `final_outputs/diagnostics` | `premerge_pose_join_ready.csv`, `premerge_pose_join_ready_summary.json` | `DOC-C13-06` |
| `#14-1` | `cells/14_01.py` | merge 本体 | `ensure_target_chunk_manifest`, `resolve_chunk_input_dir` | `persist_root`, `anchor_dir`, `global_anchor_df`, `global_camera_matrix_df`, `chunk_global_transforms_arc`, `keep_df`, `merge_summary` | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/*` | `merged_gs_arc.ply`, `merged_scene_arc.glb`, `chunk_global_transforms_arc.csv`, `chunk_keep_summary_arc.csv`, `chunk_transform_quality_arc.csv`, `owner_record_histogram_arc.csv`, `chunk_assignment_summary_arc.csv`, `merge_summary.json`, `final_output_manifest_arc.json` | `DOC-C14-01` |
...
| `MRL-10` | `-` | `frame_record.jsonl + images` を正とする record-native `Colab` route を canonical 化し、`sequence-anchor first`、`1 frame = 1 record`、`strict one-to-one join`、`adjacent continuity precheck`、`official API / CLI` 基準を [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) / [da3_ngl_runbook.ipynb](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.ipynb) の canonical pair へ固定する。camera pose / trajectory の事前推定も `DA3NESTED-GIANT-LARGE-1.1` だけで行い、`mRL-10.1` と `mRL-10.2` は `p-done`、残りは chunk route、merge 品質、runbook 構成固定である | `sd9`,`su12`,<br>`su14`,`sd11` | `bd16`,`bd17`,<br>`bd18a`,`bd20` | `td16`,`td17`,<br>`td18`,`td19a`,<br>`td23` | `active` | `active` | [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) の<br>`#1`-`#17` | `未収載` |
| `MRL-10` | `mRL-10.1` | `frame_record.jsonl`、対応 image 群、tracking / K / pose / timestamp を正に読んで、`input_frame_manifest.csv`、`input_frame_qc.csv`、`pose_conversion_check.csv`、`k_resize_check.csv`、`orientation_summary.json`、`da3_input_manifest_*.csv` を生成できる | `sd9`,`sd11` | `bd16`,`bd17`,<br>`bd20` | `td16`,`td23` | `p-done` | `p-done` | [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) の<br>`#7`-`#8` | `未収載` |
| `MRL-10` | `mRL-10.2` | proof / production を分離した sequence-anchor route が image-only 推論で通り、production 側では `frame_record` 由来の `intrinsics` / `extrinsics_w2c` を使って world point cloud と評価 summary まで保存できる。upright canonical image と legacy intrinsics 補正結果が同じ manifest 群で残る | `sd9`,`su14`,<br>`sd11` | `bd17`,`bd18`,<br>`bd20` | `td17`,`td18`,<br>`td23` | `p-done` | `p-done` | [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) の<br>`#9`-`#10` | `未収載` |
| `MRL-10` | `mRL-10.3` | `Giant` production candidate route は `continuous_gs_v06_chunk18_overlap6_adopt12` とし、canonical runbook の chunk planning section で `extrinsics_w2c_prod.npy` から全 frame の global camera matrix、`camera_matrix_full_arc.csv`、全 chunk manifest、`batch_plan.csv` を作る。`chunk_size = 18`、`step = 12`、`adopt_size = 12`、`chunks_per_batch = 3`、`process_res = 504` を config 正本で固定し、各 chunk は `6frame` 重複しつつ基本 `後半 12frame` を再構成責務とする。chunk 構築と batch 実行自体は成立したが、final merge 品質は `mRL-10.4` 未了のため全体は `active` とする | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) の<br>`#11`-`#12` | `未収載` |
| `MRL-10` | `mRL-10.4` | chunk merge は local / global の camera center だけでなく `camera_matrix_full_arc.csv` と各 chunk の `pred_extrinsics.npy` から得る camera orientation も使う。ただし現時点の主課題は owner keep 調整より前に、`pred_extrinsics` の pose convention 固定、positive similarity 強制、PLY / GLB 一致確認を閉じることである。その後に `owner_record_index` keep を再調整し、`chunk_global_transforms_arc.csv`、`vertex_assignment_summary.csv`、`owner_record_histogram_arc.csv`、`chunk_assignment_summary_arc.csv`、`merge_warning_summary_arc.json`、`chunk_transform_quality_arc.csv` を残す | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) の<br>`#13`-`#15` | `未収載` |
| `MRL-10` | `mRL-10.5` | canonical runbook pair 自体の設計を固定する。対象は [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) / [da3_ngl_runbook.ipynb](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.ipynb) であり、`#1`-`#17` の clean bootstrap 構成、section と code cell 番号一致、official API / CLI 基準、`extrinsics_w2c_arc.npy` provenance 明示、anchor 以降の各出力に対する input / output summary 表示、admin 手順と artifact 契約の同期、さらに source file から pair を同期できる authoring 面と `HAUB` 上の関数 / 変数一覧を維持する | `sd9`,`su12`,<br>`sd11` | `bd17`,`bd20` | `td16`,`td17`,<br>`td23` | `active` | `active` | [da3_ngl_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook.md) の<br>`#1`-`#17`、[da3_ngl_runbook_design_contract.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\colab\da3_ngl_runbook_design_contract.md)、admin 手順 38-45 | `未収載` |
...
| `mRL-10.4c` | 代表 chunk で `pred_extrinsics.npy` の pose convention を固定し、`w2c` / `c2w` と軸反転候補のうち、正 `scale` で global camera matrix に最も整合する解釈を選ぶ | `active` |
| `mRL-10.4d` | similarity を positive 側へ拘束し、`scale <= 0`、極小 `scale`、`center_rmse` 超過、`rotation_dir_residual` 超過を hard fail にする | `active` |
| `mRL-10.4e` | `PCA 1軸帯 keep` と terminal 全保持 fallback を削除し、`owner_record_index` ベース keep へ置き換える。owner 候補は chunk 内 record を優先し、候補不足の時だけ局所 margin / 全体 fallback へ広げる | `ready` |
| `mRL-10.4f` | GLB scene graph の node transform を保持したまま global transform を適用し、PLY と GLB の姿勢差をなくす | `ready` |

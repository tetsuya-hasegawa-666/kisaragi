...
| `mRL-10.4c` | `PCA 1軸帯 keep` と terminal 全保持 fallback を削除し、`owner_record_index` ベース keep へ置き換える | `active` |

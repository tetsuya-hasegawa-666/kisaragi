...
旧 `DA3 script 一覧表` を `modeling` 用一覧表として扱う。`camera trajectory`、`pose`、`intrinsics`、`anchor basis` の共通契約は `#6 Shared Helpers` に集約し、`#7`、`#9`、`#13`、`#14` はその helper と `01_anchor` / `manifests` / `chunk_runs` / `merged` / `final_outputs` の同一参照面で動く。加えて `#13` は `ARCore anchor baseline judge`、`#14` は `ARCore baseline route` と `DA3 predicted trajectory experimental route` の両方を比較できる merge engine として扱う。
...
| `#7matching-1` | `cells/07matching_01.py` | overlap pose matching | `resolve_matching_chunk_names`, `resolve_chunk_artifact`, `estimate_pose_aware_similarity`, `transform_c2w_list`, `plot_pose_match` | `MATCHING_CHUNK_IDS_1BASED`, `chunk_a_name`, `chunk_b_name`, `overlap_records`, `relative_rotation_deg`, `summary` | `persist_root/01_anchor/07matching`, `final_outputs/chunk_evidence`, `pipeline_root/chunk_runs` | `*_overlap_pair_metrics_arc.csv`, `*_trajectory_points_arc.csv`, `*_transform_b_to_a.npy`, `*_trajectory_match.png`, `*_matching_summary.json` | `DOC-C07M-01` |
...
| `#12-1` | `cells/12_01.py` | batch execution item 生成 | - | `execution_chunks_df`, `execution_batch_plan_df`, `batch_execution_items_df` | `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `final_outputs/diagnostics` | `batch_execution_items.csv`, `batch_execution_items_summary.json`, `/content/runbook_batch_preflight_status.json` | `DOC-C12-01` |
| `#12-2` | `cells/12_chunk_wrapper.py` | chunk 実行 wrapper | `_resolve_device`, `_pick_pred_array`, `main` | `REPO_ROOT`, `SRC_ROOT`, `export_format`, `gs_ply_saved` | `repo_root`, `src_root`, `chunk_runs_dir` | chunk ごとの `pred_extrinsics.npy`, `pred_intrinsics.npy`, `gs_ply/0000.ply`, `_SUCCESS.json` | `DOC-C12-02` |
| `#12-3` | `cells/12_03.py` | chunk-local DA3 prepose build | - | `wrapper_path`, `items_df`, `DRY_RUN`, `DEVICE`, `EXPORT_FORMAT` | `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `final_outputs/diagnostics` | run manifest、batch launch summary、chunk local pose / GS | `DOC-C12-03` |
| `#13-1` | `cells/13_01.py` | prepose graph judge | `to_4x4_batch`, `build_anchor_c2w_list`, `local_c2w_list`, `estimate_pose_aware_similarity`, `transform_c2w_list`, `pick_selected_candidate`, `summarize_relative_transform` | `anchor_dir`, `camera_anchor_full_path`, `LOCAL_EXTRINSIC_MODE`, `LOCAL_CAMERA_BASIS`, `route_label`, `fallback_used`, `candidate_df`, `validation_df`, `graph_solution_rows`, `graph_edge_rows`, `summary`, `relative_rotation_deg` | `persist_root/01_anchor`, `pipeline_root/manifests`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `final_outputs/diagnostics` | `pred_vs_anchor_pose_residual.csv`, `premerge_route_compare_arc.csv`, `premerge_route_compare_summary.json`, `premerge_pose_validation.csv`, `premerge_pose_validation.json`, `prepose_chunk_graph_solution_arc.csv`, `prepose_chunk_graph_edges_arc.csv`, `prepose_chunk_graph_summary.json` | `DOC-C13-01` |
...
| matching 契約 | `#7matching-1` | 2 chunk overlap pose を同じ座標系へ事前整合し、後段 graph / merge の前に scale、rotation、translation、pre/post residual を診断できるようにする。input 不足時は `skipped` で返し、top-to-bottom 実行を壊さない | `persist_root/01_anchor/07matching`, `final_outputs/chunk_evidence`, `pipeline_root/chunk_runs`, `MATCHING_CHUNK_*`, `relative_rotation_deg` | `*_matching_summary.json`, `*_overlap_pair_metrics_arc.csv`, `*_trajectory_points_arc.csv`, `*_trajectory_match.png`, `*_transform_b_to_a.npy` |
| judge 契約 | `#13-1` | baseline route を fallback / judge 面に残し、`DA3 predicted trajectory experimental route` を primary 候補として比較する。比較結果を route 選択だけで終わらせず、`chunk_to_world` の事前整合 graph として固定し、chunk ごとの `route_label`、`fallback_used`、overlap / non-overlap residual、chunk 間 relative transform を見えるようにする | `persist_root/01_anchor`, `pipeline_root/chunk_runs`, `pipeline_root/merged`, `route_label`, `fallback_used`, `graph_parent_chunk_name`, `t00..t33`, `relative_rotation_deg` | `premerge_pose_validation.json`, `premerge_route_compare_summary.json`, `prepose_chunk_graph_solution_arc.csv`, `prepose_chunk_graph_edges_arc.csv`, residual / gate CSV |

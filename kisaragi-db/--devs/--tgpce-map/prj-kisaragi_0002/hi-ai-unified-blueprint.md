...
| `BLK-5` | `P5` | `other` | `correcting` の短中時間帯は `3分` 収録まで停止成功を確認したが、`10min` 連続収録はまだ未確認である | 現場記録の本機能成立を `i-pass` にできない | `MRL-2S` で `10min` 実収録を行い、screen off 抑止と finalize 完了を確認する。関連 gate は `MRL-2S` | `big-open` | `hi-ai-unified-blueprint.md`, `admin-mrl-test-method.md` |
...
6. `MRL-2S` を redesigned runtime 上で再実施し、`correcting` の bounded stop 修正 build を `10min` 実収録で確認して連続収録安定化を閉じる
...
この節は `HAUB` の `TDD` 後段に置く `DA3` script inventory の入口とし、canonical pair の stage / cell 正本を要約する。

- 全件正本は `kisaragi-db/--devs/--products/prj-kisaragi_0002/colab/da3_ngl_runbook_source_inventory.md` とし、`Cell Inventory`、`Function And Class Inventory`、`Variable Inventory` の 3 section を持つ。
- この `HAUB` には stage / cell 単位の要約を置き、詳細な関数・変数の責務、validation、errors、scope は source inventory 正本で追う。
- `camera trajectory`、`pose`、`intrinsics`、`anchor basis` の共通契約は `#6 Shared Helpers` に集約し、`#7`、`#9`、`#14` はその helper を使って同じ参照面で動く。
...
| #6-1 | `cells/06_shared_helpers.py` | `direct` | 6 Shared Helpers | load_ctx, save_json, append_sequence_columns, rotmat_to_rpy_deg, _summary_rows, display_stage_summary | RUNBOOK_CTX_PATH | `DOC-C06-01` |
| #7-1 | `cells/07_01.py` | `direct` | 7 Full Anchor Build | _load_frame_records, _ranked_image_dirs, _read_actual_wh, _normalize_intrinsics_to_upright, _pick_pred_array, _save_optional_array, _run_ngl_preview_inference, _normalize_rows, _render_pose_preview | ctx, config, probe_root, anchor_dir, manifest_dir, da3_nested_dir, images_dir, frame_record_path, frame_pose_index_path, repo_root, repo_url, src_root | `DOC-C07-01` |
| #7-1-1 | `cells/07_01_01.py` | `direct` | 7 Full Anchor Build | _shape_text, _dtype_text, _minmax_text, _sample_indices, _to_u8, _save_contact_sheet | ctx, anchor_dir, preview_summary_path, preview_input_manifest_path, pred_depth_path, pred_conf_path, pred_extrinsics_path, pred_intrinsics_path, camera_anchor_full_path, preview_html_path, preview_png_path, return_inventory_csv | `DOC-C07-01-01` |
| #7-2 | `cells/07_02.py` | `direct` | 7 Full Anchor Build | build_anchor_inputs_from_zip | ctx_path, ctx, manifest_dir, da3_nested_dir, world_dir, final_outputs_dir, final_outputs_merged_dir, final_outputs_diagnostics_dir, final_outputs_manifests_dir, final_outputs_chunk_evidence_dir, images_dir, frame_record_path | `DOC-C07-02` |
| #7-3 | `cells/07_03.py` | `direct` | 7 Full Anchor Build | _existing, _find_manifest_triplet, _normalize_rows | config, search_roots, anchor_dir, manifest_dir, manifest_df, intrinsics, extrinsics_w2c, c2w, camera_centers, right_vecs, up_vecs, lens_vecs | `DOC-C07-03` |
| #7-4 | `cells/07_04.py` | `direct` | 7 Full Anchor Build | _existing, _find_anchor_root, _normalize, _wrap_deg, _angle_deg | config, search_roots, persist_root, anchor_dir, anchor_path, anchor_df, required_cols, missing, lens, up, right, centers | `DOC-C07-04` |
...
| #12-2 | `cells/12_chunk_wrapper.py` | `wrapper_body` | 12 Run Batches | _resolve_device, _pick_pred_array, _save_optional_array, main | REPO_ROOT, SRC_ROOT | `DOC-C12-02` |

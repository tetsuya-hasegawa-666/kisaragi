...
- `modeling` の stage artifact は `probe_root/final_outputs/#No/` を基本にし、各 `#No` の直下を `re_access/` と `persist_only/` に分ける。`re_access/` は後段 block や resume が再参照する manifest / pointer / summary を置き、`persist_only/` は final artifact と最低限の再解釈情報を置く。script は `persist_only/` を canonical 実体、`re_access/` を再開導線として扱い、同系統 file を 3 系統以上へ複製しない。
- `#10-1` の graph / gate 出力は `probe_root/final_outputs/#10-1/persist_only/` を canonical 保存先とし、`#10-1/re_access/graph_contract_manifest.json` が `#11-1` への handoff 入口になる。
- `#11-1` の final merge 出力は `probe_root/final_outputs/#11-1/persist_only/` を canonical 保存先とし、少なくとも `merged/merged_gs_arc.ply`、`merged/merged_scene_arc.glb`、`diagnostics/chunk_global_transforms_arc.csv`、`diagnostics/chunk_keep_summary_arc.csv`、`diagnostics/chunk_transform_quality_arc.csv`、`diagnostics/owner_record_histogram_arc.csv`、`diagnostics/chunk_assignment_summary_arc.csv`、`diagnostics/merge_warning_summary_arc.json`、`diagnostics/all_batch_summary_arc.json`、`diagnostics/merge_summary.json`、`diagnostics/merge_resume_state.json`、`diagnostics/merged_camera_pose_arc.csv`、`diagnostics/merged_camera_matrix_arc.csv`、`diagnostics/merged_camera_c2w_arc.npy`、`diagnostics/merged_extrinsics_w2c_arc.npy`、`diagnostics/ngl_pose_bundle_summary.json`、`manifests/da3_input_manifest.csv`、`manifests/camera_center_matrix_arc.csv`、`manifests/camera_matrix_full_arc.csv`、`manifests/camera_anchor_full_arc.csv`、`manifests/chunk_index_all.csv`、`manifests/batch_plan.csv`、`chunk_evidence/<chunk_name>/...`、`final_output_manifest_arc.json` を残す。`#11-1/re_access/` には resume / handoff manifest だけを置く。

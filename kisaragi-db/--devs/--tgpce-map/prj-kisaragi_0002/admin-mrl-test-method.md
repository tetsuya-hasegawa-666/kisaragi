...
19. `通常計測` のまま `10min` 連続収録を行い、画面が自動減光や screen off に入らず、収録が継続することを確認する。
20. 上の長時間収録後に `撮影停止` を押し、app が落ちずに session を finalize できることを確認する。
...
50. `merged_scene.glb` または `merged_gs.ply` を viewer で開き、天地反転していないこと、camera pose と scene の向きが一致してぶれた二重像になっていないことを確認する。異常がある時は `chunk_global_transforms.csv` の `rotation_det`、`center_rmse`、`rotation_dir_residual` を確認し、runbook の pose-aware merge 実装に従って再実行する。

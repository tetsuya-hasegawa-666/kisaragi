...
27. `DA3Metric-Large` の `Colab bootstrap` は [da3_colab_clean_bootstrap_runbook.md](C:\Users\tetsuya\kisaragi\kisaragi-db\--devs\--products\prj-kisaragi_0002\modeling\da3_colab_clean_bootstrap_runbook.md) を正本として扱う。この runbook の admin 実行は `INITRL-1` / `mINITRL-1.2`、`MRL-11` / `mRL-11.1`、`MRL-12` / `mRL-12.2` の candidate evidence を兼ねるが、`gs_model` を含む `SpacePackage` 実生成確認は別途 `MRL-12` 本体で確認する。

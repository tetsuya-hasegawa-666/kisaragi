...
| `modeling` | `MRL-3` から `MRL-6` は bootstrap、bundle 読込、single-frame `3DGS` smoke、artifact 取得まで進んでいる。`MRL-7` では `mRL-7.1` が `p-done` になり、multi-frame depth batch と world fusion による `11 frame` / `3696 points` の candidate-visible-proof まで進んだ |
...
2. `mRL-7.2` として、実 session の最長連続 window 約 `3.95s` を基準に `PLY` viewer で multi-frame 点群を目視し、全体俯瞰、時系列、camera と人の相対表示の価値確認へ進む
...
| `TraceCore` 次段 | `td19` | 次段の first target は、実 session の最長連続 window を使って `TraceCore` の最小表示へ進み、全体俯瞰、時系列、相対表示、滞留、交錯を検討できる水準へ近づけることである | `MRL-7` | `mRL-7.1` は `p-done`。次は `mRL-7.2` として `PLY` viewer 目視確認と最小表示へ進む |
...
| `MRL-7` | `mRL-7.2` | `PLY` viewer で multi-frame 点群を目視し、人軌跡重畳を含む `TraceCore` 最小表示へ進み、全体俯瞰、時系列、相対表示、滞留や交錯の兆候を検討する | `su15`,`sd11` | `bd18`,`bd20` | `td19`,`td23` | `active` | `ready` | 操作手順 27-33 と<br>`PLY` viewer 目視確認 | `未収載` |

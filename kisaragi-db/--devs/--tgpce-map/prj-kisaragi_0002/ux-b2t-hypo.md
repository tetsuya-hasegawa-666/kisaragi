...
| `BLK-12` | `continuous_gs_v06_chunk18_overlap6_adopt12` の merge が chunk 内 local camera center だけを基準に similarity 合成しており、camera orientation を使っていない | chunk trajectory がほぼ直線に近い session では回転自由度が余り、`merged_gs.ply` / `merged_scene.glb` が天地反転したり camera pose と scene がずれてぶれたように見える | `MRL-10` に `mRL-10.4` を追加し、global / local の camera matrix 両方を使う pose-aware alignment へ置き換える。rollback point は `codex/mrl10-merge-baseline-20260404` とし、center-only merge を戻せる状態を維持する |
...
6. `MRL-10` の chunk merge を pose-aware alignment へ置き換え、`merged_gs.ply` と `merged_scene.glb` が天地反転せず、camera pose と scene が同じ global 向きで再構築されることを閉じる
...
| `MRL-10` | `mRL-10.4` | chunk merge は local / global の camera center だけでなく `camera_matrix_full.csv` と各 chunk の `pred_extrinsics.npy` から得る camera orientation も使って similarity を解く。`merged_gs.ply` と `merged_scene.glb` は天地反転せず、camera pose と scene の global 向きが一致し、`chunk_global_transforms.csv` に `scale`、`rotation_det`、`center_rmse`、`rotation_dir_residual` を残せる | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Block 4-6 | `未収載` |

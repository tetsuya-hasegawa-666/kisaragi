...
| `MRL-10` | `mRL-10.3` | `Giant` production candidate route は `continuous_gs_v03` として split し、bootstrap では `DA3NESTED-GIANT-LARGE-1.1` を `infer_gs=False` / `mini_npz` で target range のみ実行して global pose を取る。その後 `chunk_size = 18`、`step = 6`、`adopt_size = 6`、`process_res = 504` の chunk 単位で `infer_gs=True` を回し、`gs_ply` / `gs_video` / `scene.glb` を chunk ごとに保存し、最後に `merged_gs.ply` と `continuousgsv03` bundle を保存できる | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Block 3-4 | `未収載` |

...
| `MRL-10` | `mRL-10.3` | `Giant` production candidate route は `continuous_gs_v03` として split し、bootstrap では `DA3NESTED-GIANT-LARGE-1.1` を `infer_gs=False` / `mini_npz` で全 chunk の global pose anchor を取る。その後 `chunk_size = 18`、`step = 6`、`adopt_size = 6`、`process_res = 504` の全 chunk manifest を固定し、`Block 4` を `3 chunk batch` ごとに繰り返し実行して `infer_gs=True` を回す。全 chunk 完了後に `merged_gs.ply` と `continuousgsv03` bundle を保存できる | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Block 3-4 | `未収載` |

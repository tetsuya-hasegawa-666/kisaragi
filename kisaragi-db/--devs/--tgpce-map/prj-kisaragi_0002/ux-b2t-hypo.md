...
| `MRL-10` | `mRL-10.3` | `Giant` production candidate route は `continuous_gs_v05_chunk3_sequential` とし、`Block 3` で `DA3NESTED-GIANT-LARGE-1.1` を `infer_gs=False` / `npz` で全 frame の global pose anchor と `camera_matrix_full.csv`、全 chunk manifest、`batch_plan.csv` を作る。その後 `chunk_size = 3`、`step = 3`、`adopt_size = 3`、`chunks_per_batch = 3`、`process_res = 504` で `Block 4` helper を読み、`Block 5` を `RUN_BATCH_INDEX` を変えながら繰り返して `infer_gs=True` を回す。最後は `Block 6` が completed chunk 全体から `merged_gs.ply` と `merged_scene.glb` を再構築し、`continuousgsv05chunk3seq` bundle を保存できる | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Block 3-6 | `未収載` |

...
| `BLK-11` | `Giant` viewer / render の image 向きが左に `90度` 倒れた状態で扱われており、upright な可読性が低い | `viewer` 確認、top camera 後続、経路重畳の読解性が落ち、`1 record` 契約に対する回転補正責務も曖昧になる | `MRL-11` を新設し、`90度右回転` を canonical orientation として image 回転、`width` / `height`、`K` 補正、manifest 記録、`proof_giant` / 後続 renderer の整合を同時に閉じる |
...
5. `MRL-11` として、左に `90度` 倒れた image / render を `90度右回転` の upright 基準へ正規化し、rotation で `K` と manifest を壊さない route を main runbook pair に加える
6. `MRL-10` / `MRL-11` の後で、`gs_ply` を top camera renderer と reviewing viewer へ接続する route、multi-view / 長時間 optimization、request / status / result download UX を整理する
...
| `MRL-10` | `modeling` | `td16`, `td17`, `td18`, `td19a`, `td23` | `frame_record.jsonl + images` を正とする record-native `Colab` route を canonical 化し、QC、proof / production 分離、評価出力、`Giant` debug bundle をそろえる | 旧 `frame_pose_index.csv` 中心 runbook を置き換え、`MetricLarge route` と `Giant route` の両方が同じ 1-record contract と manifest 群で動く main runbook pair を整える |
| `MRL-11` | `modeling` | `td17`, `td18`, `td23` | `Colab` と `viewer` の image 向きを upright 基準へ正規化し、`90度右回転` の image / `K` / manifest / export 整合を 1 route で扱えるようにする | `1 record` 構造を壊さずに orientation 問題を閉じ、`Giant` render と後続 top camera renderer が同じ向き基準で読めるようにする |
...
| `upright orientation route` | `td17`,`td18`,`td23` | `living-DA3-Giant-1.1.png` のように view が左へ `90度` 倒れると可読性が落ちる。`MRL-11` では `90度右回転` を canonical orientation として image 回転、`width` / `height` swap、`K` 補正、manifest 記録、viewer / renderer export を同時に扱う | `MRL-11` | raw image と正規化 image の両方を trace し、後続 top camera renderer の基準向きを固定する |
...
| `MRL-10` | `-` | `frame_record.jsonl + images` を正とする record-native `Colab` route を canonical 化し、`MetricLarge route` と `Giant route` の両方を同じ manifest 契約で扱える main runbook pair を成立させる。proof と production を分離し、QC、baseline thinning、`debug_gs_readback` を含む評価 manifest を備える | `sd9`,`su12`,<br>`su14`,`sd11` | `bd16`,`bd17`,<br>`bd18a`,`bd20` | `td16`,`td17`,<br>`td18`,`td19a`,<br>`td23` | `active` | `ready` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` section | `未収載` |
...
| `MRL-10` | `mRL-10.2` | proof / production を分離した `MetricLarge route` が image-only 推論で通り、production 側では `frame_record` 由来の `intrinsics` / `extrinsics_w2c` を使って world point cloud と評価 summary まで保存できる | `sd9`,`su14`,<br>`sd11` | `bd17`,`bd18`,<br>`bd20` | `td17`,`td18`,<br>`td23` | `active` | `ready` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Phase D | `未収載` |
| `MRL-10` | `mRL-10.3` | `Giant` proof route が `DA3NESTED-GIANT-LARGE-1.1`、`da3_estimated pose`、`infer_gs=True`、`process_res = 1008`、`ref_view_strategy = middle`、`num_max_points = 1250000` で通り、`gs_ply` / `gs_video` / `scene.glb` と `debug_gs_readback` bundle を保存できる | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `ready` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Block 3 | `未収載` |
| `MRL-11` | `-` | `Giant` proof image と後続 viewer / renderer の向きを upright 基準へ正規化し、左へ `90度` 倒れた render を `90度右回転` として一貫して扱える main route を作る | `su14`,`su15`,<br>`sd11` | `bd18a`,`bd20` | `td17`,`td18`,<br>`td23` | `ready` | `ready` | `未収載` | `未収載` |
| `MRL-11` | `mRL-11.1` | `Block 1` で orientation policy を持ち、raw image path、normalized image path、rotation、`width` / `height` swap、`K` 補正結果を manifest に保存できる | `sd9`,`sd11` | `bd17`,`bd20` | `td17`,`td23` | `ready` | `ready` | `未収載` | `未収載` |
| `MRL-11` | `mRL-11.2` | `MetricLarge` / `Giant` / `viewer` が同じ upright image を使い、export 画像と `gs_ply` 補助 bundle の readback が `90度右回転` 基準で読める | `su14`,`su15`,<br>`sd11` | `bd18a`,`bd20` | `td18`,`td23` | `ready` | `ready` | `未収載` | `未収載` |

...
- `tu1` から `tu16` と `td1` から `td8` は contract 実装、test、routing 実行で成立したが、admin `UX check` 未完のため対応 `MRL` / `mRL` は `active` として扱う
...
- `su14`: 利用者は、人物の動きが主空間へ同じ時刻で重ねられた結果を確認できる
- `su15`: 利用者は、見失い区間と再拘束の不確実性を理由付きで把握できる
- `su16`: 利用者は、主カメラの動きと人物の動きを同じ時刻軸で比較できる
- `su17`: 利用者は、同じ時刻の位置関係をハイライトし、注視すべき区間を絞り込める
- `su18`: 利用者は、レビューの入口だけで空間、経路、同時刻ハイライト、`attention point` を操作できる
...
- `sd7`: 運営者は、`trajectreview-modeling` から `Colab` 送信前の軽量 local sample model と handoff request を生成し、PC 上で logic を先に検証できる
- `sd8`: 運営者は、`trajectreview-modeling` だけで `DA3Metric-Large` の metric depth 推定と `ARCore pose` / intrinsics 統合の成否を確認できる
- `sd9`: 運営者は、同じ session に対して複数の sampling / intrinsics route を同一指標で比較できる
...
- `bu14`: 実行 phase は、`Next Action` を常に `完了を待つ` 1 件に保ち、現在段階を別 line で返す
- `bu15`: `DA3Metric-Large` の入力成立が難しい時は、remote modeling を開始せず `入力条件を見直す` を返す
- `bu16`: `trajectreview-modeling` は `Colab` 実行 request を export し、remote 実行結果の受理後に `SpacePackage`、`TrajectoryPackage`、`modeling_handoff_manifest.json` を更新する
- `bu17`: `SpacePackage` は、`DA3Metric-Large` の metric depth と `ARCore pose` / intrinsics による world projection から得た主空間基準、主カメラ path、空間品質を返す
- `bu18`: `TrajectoryPackage` は、主カメラ path、人物 path、不確実性、再拘束点を同じ `Timeline` 上で返す
- `bu19`: 人物 path の relink は visual match confidence、time gap、anchor proximity、`BT` 主体維持、`IMU` 連続性で判定し、不成立時は不確実性を上げる
- `bu20`: `Verify` は空間品質と経路品質を同時に返し、`Interpret` は同時刻ハイライト候補と `attention point` を返す
- `bu21`: `trajectreview-reviewing` は `ReviewArtifact` 実体を読み、viewer 操作、same-time highlight、`attention point` jump を返す
- `bu22`: `Assembly` は `3DGS` 系空間表現の操作情報、経路、同時刻ハイライト情報を束ねた `ReviewArtifact` を唯一生成する
...
- `bd11`: `trajectreview-modeling` は `SpaceReconstruction` と `TrajectoryReconstruction` に必要な gate、handoff、進行表示を主表示にする
...
- `bd14`: `trajectreview-modeling` は `session_package.json`、`sensor_quality.json`、`space_handoff_manifest.json`、`camera_calibration_summary.json` を読み、`local_model_summary.json`、`colab_job_request.json`、`review_artifact_stub.json` を生成する
- `bd15`: `trajectreview-reviewing` と統合 app は `local_model_summary.json` と `review_artifact_stub.json` を読んで verify / review 状態を組み立てる
- `bd16`: `trajectreview-modeling` は `session_package.json`、`arcore_pose.jsonl`、`frame_pose_index.csv`、`camera_calibration_summary.json`、frame 群から `DA3Metric-Large` 用の前処理入力、metric depth 推定、world projection を route 単位で実行できる
- `bd17`: `trajectreview-modeling` は sampling route、intrinsics route、ごとの quality、runtime、resource usage、failure reason を `benchmark_summary.json` へ集約できる
- `bd18`: `trajectreview-modeling` は比較結果から `selected_route.json` を生成し、採用 route と research route を分離できる
- `bd19`: `MRL` / `mRL` の `pass` は admin `UX check 完了` と本来機能の実行証跡を要件とし、`UX-only`、contract、sample、build / install は補助 gate として別記する
...
| `su10` | `bu14` | 実行 UX | `Next Action` は常に 1 件で、現在段階は補足 line に分離される |
| `su11` | `bu15` | パイプライン安全性 | 主空間再構成 failure 時に remote modeling を開始せず、入力見直し理由を返す |
| `su12` | `bu16` | modeling 本機能 | `trajectreview-modeling` だけで `Colab` request、upload 対象、remote result 受理後の package 更新を行える |
| `su13` | `bu17` | 主空間確認 | 主空間基準、主カメラ path、空間品質が読める |
| `su14` | `bu18`,`bu19` | 人物経路確認 | 人物 path が主空間へ重ねられ、不確実区間と再拘束点が識別できる |
| `su15` | `bu19` | 不確実性 | 再拘束失敗時に不確実性 mode と理由が更新される |
| `su16` | `bu18`,`bu20` | 同時刻比較 | 主カメラ経路と人物経路が同じ時刻軸で比較できる |
| `su17` | `bu20` | ハイライト | 同じ時刻の位置関係と `attention point` に時間範囲と理由が入る |
| `su18` | `bu21` | reviewing 本機能 | `trajectreview-reviewing` が実 `ReviewArtifact` を開き、経路、同時刻ハイライト、`attention point` を操作できる |
| `su19` | `bu22` | 閲覧成果物 | `ReviewArtifact` に `3DGS` 操作、経路表示、同時刻ハイライトが含まれる |
...
| `sd7` | `bd14`,`bd15` | local sample modeling | `Colab` account 未取得でも local sample model と handoff request を生成し、reviewing へ渡せる |
| `sd8` | `bd16` | `DA3` depth 基盤 | `DA3Metric-Large` により metric depth 推定と `ARCore pose` / intrinsics 統合の成否、主要指標、失敗理由を route 単位で確認できる |
| `sd9` | `bd16`,`bd17` | route 比較 | 同一 session に対し複数 route を再実行し、sampling、intrinsics、quality、runtime、resource usage、failure reason を同一形式で比較できる |
| `sd10` | `bd17`,`bd18` | route 運用化 | 採用 route と research route が分離され、既定 route を machine-readable に固定できる |
| `sd11` | `bd19` | gate 運用 | `UX-only`、contract、sample、本機能完成が別 gate として記録され、完了誤認が起きない |
...
| `tu13` | `bu10`,`bd19` | correcting calibration diagnostic separation | `camera_calibration_summary.json` が `読取試行あり成功 0 件`、`calibration export 実装前 data の可能性`、`coverage 低下` を区別して示せる | active | `kisaragi-db/--devs/--products/prj-kisaragi_0002/correcting/src/main/java/com/isensorium/app/CorrectingDataCheckService.kt` |
| `tu14` | `bu10`,`bd19` | shared camera intrinsics acquisition | `corecamera_shared_camera_trial` route の `arcore_pose.jsonl` で `captureDiagnostics.*.requested=true` が出て、intrinsics 未取得なら `request failure` として診断できる | active | `kisaragi-db/--devs/--products/prj-kisaragi_0002/correcting/src/main/java/com/isensorium/app/CoreCameraTrialRuntime.kt` |
...
| `tu18` | `bu14` | run phase single-action UX | run phase の `Next Action` が常に 1 件で、現在段階を別 line に出せる | pass | `kisaragi-db/--devs/--testcode/prj-kisaragi_0002/android-test/java/com/reviework/app/ReviewScreenControllerTest.kt` |
...
| `tu20` | `bu16` | colab handoff manifest | `modeling` app が upload 対象、job parameter、result 受理先を machine-readable に出力できる | active | `kisaragi-db/--devs/--products/prj-kisaragi_0002/app/src/main/java/com/reviework/app/LocalModelingService.kt` |
| `tu21` | `bu16` | remote result import | remote modeling 結果を受理し、`SpacePackage`、`TrajectoryPackage`、`modeling_handoff_manifest.json` を更新できる | planned | `kisaragi-db/--devs/--products/prj-kisaragi_0002/modeling/` |
| `tu22` | `bu17` | `SpacePackage` coordinate contract | 主カメラ path と主空間基準を返し、`GNSS` がない時に主 `ARCore` local 空間を唯一基準として返せる | pass | `kisaragi-db/--devs/--testcode/prj-kisaragi_0002/android-test/java/com/reviework/app/ReviewScreenControllerTest.kt` |
| `tu23` | `bu18` | `TrajectoryPackage` timeline registration | 主カメラ path、人物 path、不確実性、再拘束点を同じ時刻軸で返せる | pass | `kisaragi-db/--devs/--testcode/prj-kisaragi_0002/android-test/java/com/reviework/app/ReviewScreenControllerTest.kt` |
| `tu24` | `bu19` | relink uncertainty classifier | visual match confidence、time gap、anchor proximity、`BT` 維持情報で不確実性を決める | pass | `kisaragi-db/--devs/--testcode/prj-kisaragi_0002/android-test/java/com/reviework/app/ReviewScreenControllerTest.kt` |
| `tu25` | `bu20` | verify quality summary | `space` と `trajectory` の quality、同時刻比較の弱点、weak area を同時に返せる | pass | `kisaragi-db/--devs/--testcode/prj-kisaragi_0002/android-test/java/com/reviework/app/ReviewScreenControllerTest.kt` |
| `tu26` | `bu20` | interpret attention point synthesis | `attention point` と同時刻ハイライトに時間範囲、理由、不確実区間情報を持たせる | pass | `kisaragi-db/--devs/--testcode/prj-kisaragi_0002/android-test/java/com/reviework/app/ReviewScreenControllerTest.kt` |
| `tu27` | `bu21` | review artifact viewer | `reviewing` app が実 `ReviewArtifact` を読み、viewer と timeline 操作を提供できる | planned | `kisaragi-db/--devs/--products/prj-kisaragi_0002/reviewing/` |
| `tu28` | `bu22` | `ReviewArtifact` boundary contract | `Assembly` だけが `3DGS` 操作、経路表示、同時刻ハイライトを含む `ReviewArtifact` を生成する | pass | `kisaragi-db/--devs/--testcode/prj-kisaragi_0002/android-test/java/com/reviework/app/ReviewScreenControllerTest.kt` |
...
| `td13` | `bd14` | local sample modeling output | `modeling` app が `local_model_summary.json`、`colab_job_request.json`、`review_artifact_stub.json` を生成できる | pass | `kisaragi-db/--devs/--testcode/prj-kisaragi_0002/android-test/java/com/reviework/app/LocalModelingServiceTest.kt` |
| `td14` | `bd15` | reviewing actual bundle state | `reviewing` app と統合 app が modeling 結果を読み、verify / review 状態へ反映できる | pass | `kisaragi-db/--devs/--products/prj-kisaragi_0002/app/src/main/java/com/reviework/app/MainActivity.kt` |
| `td15` | `bd16` | `DA3Metric-Large` input manifest | `modeling` app が frame sampling、intrinsics mode、projection option を route 単位で `experiment_manifest.json` と `da3_input_manifest.json` に出力できる | active | `kisaragi-db/--devs/--products/prj-kisaragi_0002/app/src/main/java/com/reviework/app/LocalModelingService.kt` |
| `td16` | `bd16` | metric depth runner | `DA3Metric-Large` の少なくとも 1 route を `Colab` で実行し、depth と world projection 用の出力を保存できる | planned | `kisaragi-db/--devs/--products/prj-kisaragi_0002/modeling/` |
| `td17` | `bd16` | depth estimation report | `metric scale confidence`、`depth continuity`、`point count estimate`、failure reason を `depth_estimation_report.json` に正規化できる | planned | `kisaragi-db/--devs/--products/prj-kisaragi_0002/modeling/` |
...
- `MRL-2` では `tu18`、`tu19`、`tu22` を使って主空間基準と安全 gate を固めた
- `MRL-3` では `tu23` から `tu26` で人物経路、relink、不確実性、同時刻ハイライト、`attention point` を固めた
- `MRL-4` では `tu28`、`td2`、`td3`、`td4`、`td5` で成果物境界と運用 hygiene を固めた
...
- `MRL-10` では `td12`、`td13`、`tu20` で実 bundle 読込と local sample modeling を固める
- `MRL-11` では `td15` から `td17` と `tu20` で `DA3Metric-Large` 前処理、metric depth、`Colab` handoff を固める
- `MRL-12` では `td18` と `td19` で sampling / intrinsics route 比較を固める
- `MRL-13` では `td20` と `tu21` で採用 route 固定と remote result import を固める
- `MRL-14` では `td14`、`td21`、`tu27` で reviewing 実 bundle summary と実 `ReviewArtifact` viewer を固める

### 現在の見立て
- `tu1` から `tu7` と `td1` から `td8` の基礎 task は、契約実装、project 境界 scan、output routing 実行で `pass` になった
...
- `MRL-6` は実装と局所検証が通っているが、`pass` は admin `UX check` batch 実施後に再判定する
...
- `td9` から `td20` と `tu20` から `tu28` は multi-app 骨格、実 bundle summary、`local sample`、route 比較、viewer 検証としては有効だが、本機能完成の証拠としては不十分である
- 次段は `td15` から `td21` と `tu20` から `tu28` を中心に、`DA3Metric-Large` 前処理、multi-route 比較、採用 route 固定、remote result import、実 `ReviewArtifact` viewer、gate 分類を詰める
...
- `done` は、admin `UX check` が完了し、結果が `admin-mrl-test-evidence.md` に記録済みであることを示す。
- `未収載` は、まだ `admin-mrl-test-method.md` に具体手順が無いことを明示する。

### 利用者主導 gate
| MRL | mRL | gate test 項目 | 関連 story-id | 関連 behavior-id | 関連 task-id | 現在 gate | UX確認待ち | admin UX確認手順 | admin evidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| `MRL-2` | `-` | 主空間再構成の基礎線を成立させる | `su10`,`su11`,`su13` | `bu14`,`bu15`,`bu17` | `tu18`,`tu19`,`tu22` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-2` | `mRL-2.1` | 主カメラ path と空間基準を確認する | `su10`,`su13` | `bu14`,`bu17` | `tu18`,`tu22` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-2` | `mRL-2.2` | remote modeling 安全 gate を確認する | `su11` | `bu15` | `tu19` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-2` | `mRL-2.3` | 空間品質要約を確認する | `su13` | `bu17` | `tu22` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-3` | `-` | 人物経路再構成と同時刻比較の基礎線を成立させる | `su14`,`su15`,`su16`,`su17` | `bu18`,`bu19`,`bu20` | `tu23`,`tu24`,`tu25`,`tu26` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-3` | `mRL-3.1` | 人物経路の主空間登録を確認する | `su14`,`su16` | `bu18` | `tu23` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-3` | `mRL-3.2` | relink と不確実性を確認する | `su14`,`su15` | `bu19` | `tu24` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-3` | `mRL-3.3` | 同時刻ハイライトと `attention point` を確認する | `su16`,`su17` | `bu20` | `tu25`,`tu26` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-4` | `-` | 閲覧成果物境界と運用硬化を成立させる | `su19`,`sd1` | `bu22`,`bd2`,`bd3`,`bd4` | `tu28`,`td2`,`td3`,`td4`,`td5` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-4` | `mRL-4.1` | `ReviewArtifact` と viewer 境界を確認する | `su19` | `bu22` | `tu28` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
...
| `MRL-5` | `-` | correcting の intake / diagnose 基盤を成立させる | `su1`,`su2`,`sd2`,`su3` | `bu1`,`bu2`,`bu3`,`bu4`,`bu5` | `tu1`,`tu2`,`tu3`,`tu4`,`tu5` | `active` | `need` | `操作手順 1-8, pass の判断, fail の判断` | `correcting batch 定義`, `2026-03-25 \`MRL-5\` candidate evidence` |
| `MRL-5` | `mRL-5.1` | legacy alias を含む session source 読込を確認する | `su1` | `bu1` | `tu1` | `active` | `need` | `操作手順 1, 6-8` | `2026-03-25 \`MRL-5\` candidate evidence` |
| `MRL-5` | `mRL-5.2` | raw / derived 分離 export を確認する | `su1`,`sd2` | `bu2` | `tu2` | `active` | `need` | `操作手順 6-8` | `2026-03-25 \`MRL-5\` candidate evidence` |
| `MRL-5` | `mRL-5.3` | quality 数値表示付き app UI を確認する | `su1`,`su2` | `bu3`,`bu4` | `tu3`,`tu4` | `active` | `need` | `操作手順 7-8` | `2026-03-25 \`MRL-5\` candidate evidence` |
| `MRL-5` | `mRL-5.4` | 現場記録から export までの一連実行を確認する | `su3` | `bu5` | `tu5` | `planned` | `ready` | `操作手順 2-8` | `correcting batch 定義` |
| `MRL-6` | `-` | `trajectreview-correcting` の記録 + data-check と calibration 診断を成立させる | `su3`,`su4`,`su6`,`sd11` | `bu5`,`bu6`,`bu10`,`bd19` | `tu5`,`tu6`,`tu7`,`tu12`,`tu13`,`tu14` | `active` | `need` | `操作手順 1-8, pass の判断, fail の判断` | `correcting batch 定義`, `2026-03-26 \`MRL-6\` candidate evidence` |
| `MRL-6` | `mRL-6.1` | 記録停止後の最新 session 再読込を確認する | `su3` | `bu5` | `tu5` | `active` | `need` | `操作手順 4-6` | `2026-03-26 \`MRL-6\` candidate evidence` |
| `MRL-6` | `mRL-6.2` | app 内 `data-check` artifact 生成を確認する | `su3`,`su4`,`su6` | `bu5`,`bu6`,`bu10` | `tu6`,`tu12` | `active` | `need` | `操作手順 7-8` | `2026-03-26 \`MRL-6\` candidate evidence` |
| `MRL-6` | `mRL-6.3` | recommended correction 表示を確認する | `su4` | `bu6` | `tu7` | `active` | `need` | `操作手順 7-8` | `2026-03-26 \`MRL-6\` candidate evidence` |
| `MRL-6` | `mRL-6.4` | calibration capture 診断の切り分けを確認する | `su6`,`sd11` | `bu10`,`bd19` | `tu13` | `active` | `ready` | `未収載` | `2026-03-28 calibration diagnostic candidate evidence` |
| `MRL-6` | `mRL-6.5` | shared camera route の intrinsics 実収集を確認する | `su6`,`sd11` | `bu10`,`bd19` | `tu14` | `active` | `ready` | `操作手順 11-15, pass の判断` | `2026-03-28 calibration diagnostic candidate evidence` |
| `MRL-7` | `-` | `trajectreview-correcting` の Google Drive 転送を成立させる | `su5`,`sd11` | `bu7`,`bu8`,`bu9`,`bd19` | `tu8`,`tu9`,`tu10`,`tu11` | `active` | `need` | `操作手順 10-15, pass の判断, fail の判断` | `correcting batch 定義`, `2026-03-27 \`MRL-7\` candidate evidence` |
| `MRL-7` | `mRL-7.1` | `1 回以上 data-check` 後の転送 gate を確認する | `su5` | `bu7`,`bu8` | `tu8`,`tu9` | `active` | `need` | `操作手順 8-13` | `2026-03-27 \`MRL-7\` candidate evidence` |
| `MRL-7` | `mRL-7.2` | Google Drive への session 転送を確認する | `su5` | `bu8` | `tu10` | `active` | `need` | `操作手順 10-13` | `2026-03-27 \`MRL-7\` candidate evidence` |
| `MRL-7` | `mRL-7.3` | Drive folder 同期 contract を確認する | `su5` | `bu9` | `tu11` | `active` | `need` | `操作手順 10-13` | `2026-03-27 \`MRL-7\` candidate evidence` |

### 利用者手動の補助 UX gate
| INITRL | mINITRL | 補助 UX test 項目 | 関連 story-id | 関連 behavior-id | 関連 task-id | 現在 gate | UX確認待ち | admin UX確認手順 | admin evidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `INITRL-1` | `-` | `Colab all-in modeling` package の bootstrap 導線を成立させる | `sd6`,`sd7`,`su12` | `bd13`,`bd14`,`bu16` | `td12`,`td13`,`tu20` | `active` | `need` | `modeling batch 操作手順 1-6` | `modeling batch 定義` |
| `INITRL-1` | `mINITRL-1.1` | 抽出済み bundle snapshot を手動で選んで読めることを確認する | `sd6` | `bd13` | `td12` | `pass` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `INITRL-1` | `mINITRL-1.2` | local sample modeling と handoff request を手動で作れることを確認する | `sd7`,`su12` | `bd14`,`bu16` | `td13`,`tu20` | `active` | `need` | `modeling batch 操作手順 4-6` | `2026-03-25 \`MRL-10\` candidate evidence` |
...
| `INITRL-2` | `mINITRL-2.1` | project 境界 scan と dependency hygiene を確認する | `sd1` | `bd2` | `td2`,`td3` | `pass` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `INITRL-2` | `mINITRL-2.2` | 4 app module build と初期表示導線を確認する | `sd4`,`sd5` | `bd9`,`bd10`,`bd11`,`bd12` | `td9`,`td10`,`td11` | `pass` | `ready` | `未収載` | `2026-03-25 candidate evidence` |

### 運営者主導 gate
| MRL | mRL | gate test 項目 | 関連 story-id | 関連 behavior-id | 関連 task-id | 現在 gate | UX確認待ち | admin UX確認手順 | admin evidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `MRL-8` | `-` | `SpaceReconstruction` handoff bundle を成立させる | `sd3`,`su3` | `bd5`,`bd6`,`bd7`,`bd8`,`bu5` | `td6`,`td7`,`td8`,`tu5` | `active` | `need` | `modeling batch 前提確認 1-4` | `modeling batch 定義` |
| `MRL-8` | `mRL-8.1` | raw video を含む bundle 維持を確認する | `sd3` | `bd5` | `td6` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-8` | `mRL-8.2` | `SessionPackage` と handoff manifest 正規化を確認する | `sd3` | `bd6`,`bd7` | `td7` | `active` | `need` | `modeling batch 前提確認 1-4` | `modeling batch 定義` |
| `MRL-8` | `mRL-8.3` | handoff 可否 summary を確認する | `sd3`,`su3` | `bd8`,`bu5` | `td8`,`tu5` | `active` | `need` | `modeling batch 前提確認 1-4` | `modeling batch 定義` |
| `MRL-9` | `-` | 4 app の役割分担 UX を成立させる | `sd4`,`sd5`,`sd11` | `bd9`,`bd10`,`bd11`,`bd12`,`bd19` | `td9`,`td10`,`td11`,`td21` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
...
| `MRL-9` | `mRL-9.3` | 統合 app での境界説明と gate 分類を確認する | `sd5`,`sd11` | `bd12`,`bd19` | `td11`,`td21` | `active` | `ready` | `未収載` | `2026-03-25 candidate evidence` |
| `MRL-10` | `-` | local sample modeling と実 bundle 読込を成立させる | `sd6`,`sd7`,`su12` | `bd13`,`bd14`,`bu16` | `td12`,`td13`,`tu20` | `active` | `need` | `modeling batch 操作手順 1-6` | `modeling batch 定義`, `2026-03-25 \`MRL-10\` candidate evidence` |
| `MRL-10` | `mRL-10.1` | 実 bundle snapshot 読込を確認する | `sd6` | `bd13` | `td12` | `active` | `need` | `modeling batch 操作手順 1-3` | `2026-03-25 \`MRL-10\` candidate evidence` |
| `MRL-10` | `mRL-10.2` | local sample modeling 生成を確認する | `sd7` | `bd14` | `td13` | `active` | `need` | `modeling batch 操作手順 4-6` | `2026-03-25 \`MRL-10\` candidate evidence` |
| `MRL-10` | `mRL-10.3` | `Colab` handoff request export を確認する | `su12`,`sd7` | `bu16`,`bd14` | `tu20`,`td13` | `active` | `need` | `modeling batch 操作手順 4-6` | `2026-03-25 \`MRL-10\` candidate evidence` |
| `MRL-11` | `-` | `DA3Metric-Large` 前処理と metric depth route を成立させる | `su12`,`sd8`,`sd11` | `bu16`,`bd16`,`bd19` | `td15`,`td16`,`td17`,`tu20`,`td21` | `active` | `need` | `modeling batch 操作手順 7-14, pass の判断, fail の判断` | `modeling batch 定義`, `2026-03-28 \`MRL-11\` candidate evidence` |
| `MRL-11` | `mRL-11.1` | `DA3` input manifest と route export を確認する | `sd8` | `bd16` | `td15` | `active` | `need` | `modeling batch 操作手順 7-9` | `2026-03-28 \`MRL-11\` candidate evidence` |
| `MRL-11` | `mRL-11.2` | `Colab` 上の metric depth 実行を確認する | `sd8`,`su12` | `bd16`,`bu16` | `td16`,`tu20` | `planned` | `need` | `modeling batch 操作手順 10-14` | `modeling batch 定義` |
| `MRL-11` | `mRL-11.3` | `depth_estimation_report.json` 正規化を確認する | `sd8`,`sd11` | `bd16`,`bd19` | `td17`,`td21` | `planned` | `ready` | `未収載` | `2026-03-28 \`MRL-11\` candidate evidence` |
| `MRL-12` | `-` | multi-route 比較と benchmark 集約を成立させる | `sd9`,`sd11` | `bd16`,`bd17`,`bd19` | `td18`,`td19`,`td21` | `active` | `ready` | `未収載` | `2026-03-28 \`MRL-12\` candidate evidence` |
| `MRL-12` | `mRL-12.1` | sampling route 比較を確認する | `sd9` | `bd17` | `td18` | `active` | `ready` | `未収載` | `2026-03-28 \`MRL-12\` candidate evidence` |
| `MRL-12` | `mRL-12.2` | intrinsics / projection route 比較を確認する | `sd9` | `bd17` | `td19` | `planned` | `ready` | `未収載` | `2026-03-28 \`MRL-12\` candidate evidence` |
| `MRL-13` | `-` | 採用 route の運用化と remote result import を成立させる | `su12`,`sd10`,`sd11` | `bu16`,`bd18`,`bd19` | `td20`,`tu21`,`td21` | `active` | `ready` | `未収載` | `2026-03-28 \`MRL-13\` candidate evidence` |
| `MRL-13` | `mRL-13.1` | `selected_route.json` 生成を確認する | `sd10` | `bd18` | `td20` | `active` | `ready` | `未収載` | `2026-03-28 \`MRL-13\` candidate evidence` |
| `MRL-13` | `mRL-13.2` | remote result import と package 更新を確認する | `su12`,`sd10` | `bu16`,`bd18` | `tu21`,`td20` | `planned` | `ready` | `未収載` | `2026-03-28 \`MRL-13\` candidate evidence` |
| `MRL-14` | `-` | reviewing 本機能を成立させる | `sd6`,`sd7`,`su18`,`sd11` | `bd15`,`bu21`,`bd19` | `td14`,`tu27`,`td21` | `planned` | `ready` | `未収載` | `reviewing batch 定義` |
| `MRL-14` | `mRL-14.1` | modeling 結果の reviewing 読込を確認する | `sd6` | `bd15` | `td14` | `active` | `ready` | `未収載` | `2026-03-25 \`MRL-14\` candidate evidence` |
| `MRL-14` | `mRL-14.2` | 実 `ReviewArtifact` viewer 操作を確認する | `su18` | `bu21` | `tu27` | `planned` | `ready` | `未収載` | `reviewing batch 定義` |
| `MRL-14` | `mRL-14.3` | gate 分類を含む reviewing closeout を確認する | `sd7`,`sd11` | `bd15`,`bd19` | `td14`,`td21` | `planned` | `ready` | `未収載` | `reviewing batch 定義` |

...
| `MRL-10` | `mRL-10.3` | `Giant` proof route が `DA3NESTED-GIANT-LARGE-1.1`、`da3_estimated pose`、`infer_gs=True`、`process_res = 1008`、`ref_view_strategy = middle`、`num_max_points = 1250000` で通り、`gs_ply` / `gs_video` / `scene.glb` と `debug_gs_readback` bundle を保存できる。render / manifest / viewer 補助 bundle は upright 基準で読める | `sd9`,`su14`,<br>`sd11` | `bd18a`,`bd20` | `td19a`,`td23` | `active` | `active` | da3_colab_<br>evid_runbook.md<br>の `MRL-10` Block 3 | `未収載` |

...
この文書は `prj-kisaragi_0001` の artifact 正本を統合した `project-core.md` とする。

## 前提

- `prj-kisaragi_0001` は `codev-viewer` の実装資産を基底に、kisaragi 向けの viewer contract と名称へ再構成した project とする。

## 目的

`prj-kisaragi_0001` は live workspace を read-only で閲覧する dual-pane viewer であり、`db-view` で `kisaragi-db` tree、`prj-view` で `kisaragi-tree` tree を比較しながら、構造確認、文書読解、比較対象の選定、Explorer handoff を高速に行うために存在する。
...
- `kisaragi_0001-launch.cmd` は `kisaragi/` 直下を表示 root とする

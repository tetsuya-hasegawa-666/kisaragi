...
この文書は `prj-direview` の artifact 正本を統合した `project-core.md` とする。

## 目的

`prj-direview` は live workspace を read-only で閲覧する dual-pane viewer であり、`db-view` で `kisaragi-db` tree、`prj-view` で `kisaragi-tree` tree を比較しながら、構造確認、文書読解、比較対象の選定、Explorer handoff を高速に行うために存在する。
...
- `direview-launch.cmd` は `kisaragi/` 直下を表示 root とする

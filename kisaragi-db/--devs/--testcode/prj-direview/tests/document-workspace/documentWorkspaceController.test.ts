...
import { DocumentWorkspaceController } from "../../../../--products/prj-direview/src/document-workspace/controller/DocumentWorkspaceController";
import type { DocumentRecord } from "../../../../--products/prj-direview/src/document-workspace/model/DocumentRecord";
import type { DocumentRepository } from "../../../../--products/prj-direview/src/document-workspace/model/DocumentRepository";
...
describe("DocumentWorkspaceController", () => {
  it("returns available profiles and defaults to codev-view", () => {
    const controller = new DocumentWorkspaceController(
      new StubDocumentRepository([
        createDocument("db-a", "codev-db", "codev-db basis", "--docs/--artifact/prj-direview/north_star.md"),
        createDocument("view-a", "codev-view", "codev-view basis", "prj-direview/README.md")
      ])
    );
  ...
  it("prefers prj-view then db-view when launcher profiles are present", () => {
    const controller = new DocumentWorkspaceController(
      new StubDocumentRepository([
        ...
        createDocument("tree-a", "prj-view", "prj-view", "prj-direview/--plans/plan.md")
      ])
    );
  ...
  it("builds a nested tree for the active profile only", () => {
    const controller = new DocumentWorkspaceController(
      new StubDocumentRepository([
        createDocument("db-a", "codev-db", "codev-db basis", "--docs/--artifact/prj-direview/north_star.md"),
        createDocument("db-b", "codev-db", "codev-db basis", "--docs/--operations/prj-direview/change_protocol.md"),
        createDocument("view-a", "codev-view", "codev-view basis", "prj-direview/README.md")
      ])
    );
    ...
    expect(state.tree[0]?.children[0]?.children[0]?.label).toBe("prj-direview");
  });

  it("expands ancestor directories for the selected path", () => {
    const controller = new DocumentWorkspaceController(
      new StubDocumentRepository([
        createDocument("tree-a", "prj-view", "prj-view", "kisaragi/kisaragi-tree/prj-direview/--plans/plan.md")
      ])
    );
    ...
    const state = controller.createSelectionState(
      "prj-view",
      [],
      "kisaragi/kisaragi-tree/prj-direview/--plans/plan.md"
    );

    expect(state.expandedPaths).toEqual([
      "kisaragi",
      "kisaragi/kisaragi-tree",
      "kisaragi/kisaragi-tree/prj-direview",
      "kisaragi/kisaragi-tree/prj-direview/--plans"
    ]);
  });
  ...
  it("does not force-expand the selected directory itself", () => {
    const controller = new DocumentWorkspaceController(
      new StubDocumentRepository([
        ...
        createDirectory("tree-leaf", "prj-view", "prj-view", "kisaragi/kisaragi-tree/prj-direview")
      ])
    );

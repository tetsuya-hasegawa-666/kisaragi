...
import {
  ...
  resolveRevealTarget
} from "../../../../--products/prj-kisaragi_0001/tools/liveProjectSnapshot";
import { resolveManifestPath } from "../../../../--products/prj-kisaragi_0001/tools/resolveManifestPath";
...
describe("liveProjectSnapshot", () => {
  it("reads documents from multiple source profiles", () => {
    ...
    expect(snapshot.profiles[0]?.documents.map((document) => document.path)).toContain(
      "--docs/--artifact/prj-kisaragi_0001"
    );
    expect(snapshot.profiles[0]?.documents.map((document) => document.path)).toContain(
      "--docs/--artifact/prj-kisaragi_0001/north_star.md"
    );
    expect(snapshot.profiles[1]?.documents.map((document) => document.path)).toContain("prj-kisaragi_0001");
    expect(snapshot.profiles[1]?.documents.map((document) => document.path)).toContain(
      "prj-kisaragi_0001/README.md"
    );
    expect(snapshot.profiles[1]?.documents.map((document) => document.path)).toContain(
      "prj-kisaragi_0001/.env"
    );
    expect(snapshot.profiles[1]?.documents.map((document) => document.path)).toContain(
      "prj-kisaragi_0001/index.ts"
    );
  });
  ...
  it("reads documents from launcher-generated db-view and prj-view profiles", () => {
    ...
    expect(snapshot.profiles[1]?.documents.map((document) => document.path)).toContain("kisaragi/kisaragi-tree/prj-kisaragi_0001");
  ...
  it("resolves reveal target inside configured roots", () => {
    ...
    expect(target.relativePath).toBe("prj-kisaragi_0001/README.md");
  });
  ...
  it("prefers environment manifest path when provided", () => {
    const resolved = resolveManifestPath("C:/workspace/prj-kisaragi_0001", {
  ...
  it("falls back to the project config manifest path", () => {
    ...
    expect(resolved).toBe(join("C:/workspace/prj-kisaragi_0001", "config", "project-manifest.json"));
  });
});
...
function createProjectFixture(): { manifestPath: string } {
  ...
  mkdirSync(join(root, "codev-db", "--docs", "--artifact", "prj-kisaragi_0001"), { recursive: true });
  mkdirSync(join(root, "codev-db", "--process", "--state", "prj-kisaragi_0001"), { recursive: true });
  mkdirSync(join(root, "codev-view", "prj-kisaragi_0001"), { recursive: true });
  mkdirSync(join(root, "codev-view", "prj-isensorium"), { recursive: true });
  mkdirSync(join(root, "codev-view", "outside"), { recursive: true });

  writeFileSync(
    join(root, "codev-db", "--docs", "--artifact", "prj-kisaragi_0001", "north_star.md"),
    "# North Star\nBody\n",
    "utf8"
  );
  writeFileSync(
    join(root, "codev-db", "--process", "--state", "prj-kisaragi_0001", "current_state.md"),
    "# Current State\nBody\n",
    "utf8"
  );
  writeFileSync(join(root, "codev-view", "prj-kisaragi_0001", "README.md"), "# View\nBody\n", "utf8");
  writeFileSync(join(root, "codev-view", "prj-kisaragi_0001", ".env"), "API_KEY=test\n", "utf8");
  writeFileSync(join(root, "codev-view", "prj-kisaragi_0001", "index.ts"), "export const x = 1;\n", "utf8");
  writeFileSync(
    ...
    JSON.stringify(
      {
        ...
        sourceProfiles: [
          ...
          },
          {
            ...
            documentRoots: ["."],
            compareRoots: ["prj-kisaragi_0001", "prj-isensorium"]
          }
        ],

...
class Da3IncreposeSourcesTest(unittest.TestCase):
    ...
    def test_incremental_pose_contract_is_explicit(self) -> None:
        ...
        anchor_source = (SOURCE_DIR / "cells" / "07_02.py").read_text(encoding="utf-8")
        ...
        context_source = (SOURCE_DIR / "cells" / "04_01.py").read_text(encoding="utf-8")
        wrapper_source = (SOURCE_DIR / "cells" / "08_08.py").read_text(encoding="utf-8")
        ...
        self.assertIn('"PIPELINE_SLUG": "runtime_workspace"', config_source)
        ...
        self.assertIn('camera_anchor_full_df[f"w2c_{r}{c}"] = extrinsics_w2c[:, r, c]', anchor_source)
        self.assertIn('chunk_csv_path = chunk_manifest_dir / f"{chunk_name}.csv"', chunk_build_source)
        self.assertIn('chunk_execution_plan_path = chunk_manifest_dir / "chunk_execution_plan.csv"', chunk_build_source)
        ...
        self.assertIn('chunk_execution_plan_path = chunk_manifest_dir / "chunk_execution_plan.csv"', precheck_source)
        self.assertIn('chunk_input_manifest_path = chunk_manifest_dir / "chunk_input_manifest_arc.csv"', precheck_source)
        self.assertIn('source_label = "chunk_execution_plan"', precheck_source)
        self.assertIn('stage_10_reaccess_dir = stage_10_dir / "re_access"', path_source)
        self.assertIn('stage_11_persist_only_dir = stage_11_dir / "persist_only"', path_source)
        self.assertIn('"stage_10_reaccess_dir": str(stage_10_reaccess_dir)', path_source)
        self.assertIn('stage_10_reaccess_dir = Path(paths["stage_10_reaccess_dir"])', context_source)
        self.assertIn('stage_11_persist_only_dir = Path(paths["stage_11_persist_only_dir"])', context_source)
        self.assertIn("def build_target_chunk_and_batch_plan():", target_resolve_source)
        self.assertIn('chunk_execution_plan_path = chunk_manifest_dir / "chunk_execution_plan.csv"', target_resolve_source)
        ...
        self.assertIn('if {"w2c_00","w2c_01","w2c_02","w2c_03","w2c_10","w2c_11","w2c_12","w2c_13","w2c_20","w2c_21","w2c_22","w2c_23","w2c_30","w2c_31","w2c_32","w2c_33"}.issubset(df.columns):', wrapper_source)
        self.assertIn('if pred_ext is None and extrinsics is not None:', wrapper_source)
        self.assertIn('pred_ext = extrinsics', wrapper_source)
        self.assertIn('if batch_work_dir.name == chunk_name or batch_work_dir.name.startswith(f"{chunk_name}_"):', exec_source)
        ...
        self.assertIn("chunk_execution_plan_path = chunk_manifest_dir / \"chunk_execution_plan.csv\"", graph_source)
        ...
        self.assertIn('stage_10_reaccess_dir = Path(ctx.get("stage_10_reaccess_dir"', graph_source)
        self.assertIn('graph_gate_report_path = stage_10_persist_only_dir / "graph_gate_report.json"', graph_source)
        self.assertIn('graph_contract_manifest_path = stage_10_reaccess_dir / "graph_contract_manifest.json"', graph_source)
        self.assertIn('gate_df.to_csv(validation_csv, index=False, encoding="utf-8")', graph_source)
        self.assertIn('if batch_work_dir.name == chunk_name or batch_work_dir.name.startswith(f"{chunk_name}_"):', graph_source)
        self.assertIn('if "chunk_out_dir" in item_row.index', graph_source)
        self.assertIn('candidates.extend(sorted(chunk_runs_dir.glob(f"**/{chunk_name}*/pred_extrinsics.npy")))', graph_source)
        self.assertIn('candidates.extend(sorted(pred_path.parent.glob("chunk_input_frames*.csv")))', graph_source)
        self.assertIn('graph_solution_csv = stage_10_persist_only_dir / "prepose_chunk_graph_solution_arc.csv"', graph_source)
        self.assertIn("identity_transform_csv = chunk_manifest_dir / \"chunk_global_transforms_arc.csv\"", graph_source)
        self.assertIn('anchor_value_cols = ["cx_world", "cy_world", "cz_world", "anchor_lens_x", "anchor_lens_y", "anchor_lens_z", "anchor_up_x", "anchor_up_y", "anchor_up_z"]', merge_source)
        self.assertIn('chunk_merge_df = chunk_df.drop(columns=[c for c in anchor_value_cols if c in chunk_df.columns], errors="ignore")', merge_source)
        self.assertIn('anchor_merge_df = global_anchor_df[anchor_join_cols + anchor_value_cols].copy()', merge_source)
        self.assertIn('def has_detailed_prepose_solution(solution: dict | None) -> bool:', merge_source)
        self.assertIn('if has_detailed_prepose_solution(prepose_solution):', merge_source)
        self.assertIn('graph_persist_only_dir = Path(ctx.get("stage_10_persist_only_dir"', merge_source)
        self.assertIn('merge_persist_only_dir = Path(ctx.get("stage_11_persist_only_dir"', merge_source)
        self.assertIn('chunk_execution_plan_path = chunk_manifest_dir / "chunk_execution_plan.csv"', merge_source)
        self.assertIn('graph_gate_report_path = graph_persist_only_dir / "graph_gate_report.json"', merge_source)
        self.assertIn('premerge_pose_validation_path = Path(graph_artifacts.get("premerge_pose_validation_json"', merge_source)
        self.assertIn('merge_output_report_path = merge_persist_only_dir / "merge_output_report.json"', merge_source)
        self.assertIn('merge_input_report_path = final_outputs_manifests_dir / "merge_input_report.json"', merge_source)
        self.assertIn('stage_11_2_manifest_path = stage_11_2_dir / "11_2_handoff_manifest.json"', merge_source)
        self.assertIn('stage_access_index_path = merge_reaccess_dir / "stage_access_index.json"', merge_source)
    ...
    def test_docs_reference_increpose_canonical_pair(self) -> None:
        ...
        self.assertIn("runtime_workspace/chunk_runs/<batch_name>/<chunk_name>/pred_extrinsics.npy", self.haub)
        self.assertIn("support-MRL-S1", self.haub)
        self.assertIn("chunk_execution_plan.csv", self.haub)
        self.assertIn("graph_gate_report.json", self.haub)
        self.assertIn("merge_output_report.json", self.haub)
        self.assertIn("runtime_workspace", self.haub)
        self.assertIn("final_outputs/#10-1/persist_only/premerge_pose_validation.json", self.haub)
        self.assertIn("final_outputs/#11-1/persist_only/diagnostics/merged_camera_pose_arc.csv", self.haub)
        self.assertIn("access manifest は管理正本ではない", self.haub)
        self.assertIn("管理正本は `HAUB`", self.contract)

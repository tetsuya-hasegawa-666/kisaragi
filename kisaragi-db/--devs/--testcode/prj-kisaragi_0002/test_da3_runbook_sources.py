...
class Da3RunbookSourcesTest(unittest.TestCase):
    ...
    def test_premerge_pose_gate_contract_is_produced_and_consumed(self) -> None:
        ...
        probe_source = (SOURCE_DIR / "cells" / "13_02.py").read_text(encoding="utf-8")
        split_probe_source = (SOURCE_DIR / "cells" / "13_03.py").read_text(encoding="utf-8")
        raw_probe_source = (SOURCE_DIR / "cells" / "13_04.py").read_text(encoding="utf-8")
        arcore_validation_source = (SOURCE_DIR / "cells" / "13_05.py").read_text(encoding="utf-8")
        join_ready_source = (SOURCE_DIR / "cells" / "13_06.py").read_text(encoding="utf-8")
        ...
        self.assertIn('probe_json = merged_dir / "premerge_pose_probe_summary.json"', probe_source)
        self.assertIn('"route": "continuous-gs-v06-chunk18-overlap6-adopt12-premerge-pose-probe"', probe_source)
        self.assertIn('basis_candidates = {', probe_source)
        self.assertIn('("yaw_deg_record", "pitch_deg_record", "roll_deg_record")', probe_source)
        self.assertIn("lens_from_yaw_pitch_deg", probe_source)
        self.assertIn("rotate_about_axis", probe_source)
        self.assertIn("np.nan_to_num", probe_source)
        self.assertIn('reason": "svd_did_not_converge"', probe_source)
        self.assertIn('"probe_error": exc.args[0] if exc.args else "probe_failed"', probe_source)
        self.assertIn('split_json = merged_dir / "premerge_pose_probe_split_summary.json"', split_probe_source)
        self.assertIn('"route": "continuous-gs-v06-chunk18-overlap6-adopt12-premerge-pose-probe-split"', split_probe_source)
        self.assertIn('"forward_error_deg_p95"', split_probe_source)
        self.assertIn('"up_error_deg_p95"', split_probe_source)
        self.assertIn('inspection_json = merged_dir / "premerge_pose_raw_orientation_inspection_summary.json"', raw_probe_source)
        self.assertIn('"route": "continuous-gs-v06-chunk18-overlap6-adopt12-premerge-pose-raw-inspection"', raw_probe_source)
        self.assertIn('"sample_count_per_chunk_max": 5', raw_probe_source)
        self.assertIn('"anchor_fx"', raw_probe_source)
        self.assertIn('"pred_fx"', raw_probe_source)
        self.assertIn('summary_json = merged_dir / "arcore_anchor_trajectory_validation_summary.json"', arcore_validation_source)
        self.assertIn('anchor_dir = persist_root / "01_anchor"', arcore_validation_source)
        self.assertIn('camera_anchor_full_path = anchor_dir / "camera_anchor_full_arc.csv"', arcore_validation_source)
        self.assertIn('anchor_df["cx_world"] = anchor_df["cam_cx"]', arcore_validation_source)
        self.assertIn('"route": "continuous-gs-v06-chunk18-overlap6-adopt12-arcore-anchor-validation"', arcore_validation_source)
        self.assertIn('join_ready_csv = merged_dir / "premerge_pose_join_ready.csv"', join_ready_source)
        self.assertIn('anchor_dir = persist_root / "01_anchor"', join_ready_source)
        self.assertIn('camera_anchor_full_path = anchor_dir / "camera_anchor_full_arc.csv"', join_ready_source)
        self.assertIn('camera_matrix_full_path = anchor_dir / "camera_matrix_full_arc.csv"', join_ready_source)
        self.assertIn('matrix_full_df = matrix_full_df.merge(', join_ready_source)
        ...
        self.assertIn('premerge_pose_probe_path = merged_dir / "premerge_pose_probe_summary.json"', merge_source)
        self.assertIn('run #13-2 pre-merge pose probe before #14-1 merge', merge_source)
        self.assertIn('run #13-3 pre-merge pose split probe before #14-1 merge', merge_source)
        self.assertIn('run #13-4 pre-merge raw orientation inspection before #14-1 merge', merge_source)
        self.assertIn('run #13-5 arcore anchor trajectory validation before #14-1 merge', merge_source)
        self.assertIn('run #13-6 pre-merge join-ready data build before #14-1 merge', merge_source)
        ...
        self.assertIn("`#13-2`", markdown_source)
        self.assertIn("`#13-3`", markdown_source)
        self.assertIn("`#13-4`", markdown_source)
        self.assertIn("`#13-5`", markdown_source)
        self.assertIn("`#13-6`", markdown_source)

...
class Da3RunbookSourcesTest(unittest.TestCase):
    ...
    def test_wrapper_markdown_preserves_backslash_sequences(self) -> None:
        ...
        match = pattern.search(self.markdown)
        self.assertIsNotNone(match, "#7-4")
        block = match.group(0)
    ...
    def test_premerge_pose_gate_contract_is_produced_and_consumed(self) -> None:
        ...
        self.assertIn("# inactive note", matching_markdown)
        self.assertIn("`#7-6`", matching_markdown)
        self.assertIn("active runbook では使わない", matching_markdown)
        ...
        self.assertIn('validation_json = merged_dir / "premerge_pose_validation.json"', graph_build_source)
        self.assertIn('anchor_dir = persist_root / "01_anchor"', graph_build_source)
        self.assertIn('camera_anchor_full_path = anchor_dir / "camera_anchor_full_arc.csv"', graph_build_source)
        self.assertIn('LOCAL_EXTRINSIC_MODE = "c2w"', graph_build_source)
        self.assertIn('LOCAL_CAMERA_BASIS[:3, :3] = np.array([', graph_build_source)
        self.assertIn('ROUTE_ARCORE = "arcore_anchor_baseline"', graph_build_source)
        self.assertIn('ROUTE_DA3 = "da3_predicted_primary"', graph_build_source)
        self.assertIn('anchor_df = chunk_df.merge(', graph_build_source)
        self.assertIn('baseline_T, baseline_align = estimate_pose_aware_similarity(local_rows, global_rows)', graph_build_source)
        self.assertIn('experimental_source = "predicted_overlap"', graph_build_source)
        self.assertIn('selected_candidate["preferred_route_label"] = PREFERRED_ROUTE_LABEL', graph_build_source)
        ...
        self.assertIn('"selected_route_counts": route_counts', graph_build_source)
        self.assertIn('"fallback_count": fallback_count', graph_build_source)
        self.assertIn('"preferred_fallback_used_count"', graph_build_source)
        self.assertIn('"hard_fail_count": int(len(hard_fail_df))', graph_build_source)
        self.assertIn('save_json(final_outputs_diagnostics_dir / "premerge_pose_validation.json", summary)', graph_build_source)
        ...
        self.assertIn('graph_solution_csv = merged_dir / "prepose_chunk_graph_solution_arc.csv"', validation_source)
        self.assertIn('"route": "continuous-gs-v06-chunk18-overlap6-adopt12-prepose-graph-review"', validation_source)
        self.assertIn('assert status == "ok", validation', validation_source)

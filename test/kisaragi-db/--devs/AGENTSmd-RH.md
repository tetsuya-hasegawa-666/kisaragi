...
- 対処方法: `test/AGENTS.md` を新しい章立てへ全面更新し、directory、gate、文書運用、safety は shared rule に残し、詳細手順は skill / script / reference 側へ逃がす構造へ整理した。
- 対応内容: `test/kisaragi-skills/agents.md` と skill 系の設計文書が新主線に追随できるよう、manager 群、参照先一覧、refresh target skill 一覧も同 task で同期する前提へ切り替えた。
- 更新結果: 今後の `test/` 配下文書は、`receipt-manager -> skill-invoker -> skill-planner -> response-manager` を前提とする軽量 shared governance 構造で読める。
- 新旧比較:

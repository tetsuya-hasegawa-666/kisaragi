...
description: script、notebook、runbook を設計駆動で新規作成または再構成する。責務分離、stage 分割、設計審査票、関数 / クラス一覧表、docs ID 対応、validation / error handling 契約を先に固定してから実装したい時に使う。
---
...
7. その後にだけ code を書く。code は docs 契約から逸脱しないように保つ。
8. 変更後は `references/review-checklist.md` で責務逸脱、暗黙状態、docs 不整合、validation 抜けを点検する。

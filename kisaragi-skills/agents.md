...
- `skill-distributor` を開発 prompt の最上位入口とし、今回使うべき最小 skill 群と適用順を先に決めるものとする。
- `phase-task-orchestrator` を複数 task 開発依頼の既定入口とし、authoritative doc check、phase header、完了条件、closeout を先に固定するものとする。
...
- `design-first-script-builder`、`reference-rewire-operator`、`documentation-watchkeeper`、`delivery-planning-keeper` は `phase-task-orchestrator` の各 phase で併用する専門 skill とする。

...
```mermaid
flowchart LR
    A["rule-snapshot-reader"] -->|先に定義| E["先行候補"]
    B["authoritative-doc-scope-resolver"] -->|先に定義| E
    C["rule-diff-clarifier"] -->|先に定義| E
    D["task-intent-normalizer"] -->|先に定義| E
    F["close-condition-definer"] -->|先に定義| E
```
